...
使用方法:
    julia server_full.jl [port]

默认端口: 8080

示例:
    julia server_full.jl          # 使用默认端口8080
    julia server_full.jl 8081     # 使用端口8081
"""
...
# 加载HTTP服务器模块（使用绝对路径）
include(joinpath(@__DIR__, "src", "simulation", "MomentumMapping.jl"))
...
try
    ...
    if e isa Base.IOError || e isa ArgumentError
        ...
        println("\n💡 尝试使用其他端口:")
        println("   julia server_full.jl 8081")
        rethrow(e)
    else
        rethrow(e)
    end
