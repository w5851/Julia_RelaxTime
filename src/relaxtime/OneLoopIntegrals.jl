...
const DEFAULT_CUTOFF = 3.05
const MIN_LOG_ARG = 1.0e-20
const MAX_LOG_ARG = 1.0e20
const EPS_K = 1.0e-9
const EPS_LAMBDA = 1.0e-9
const EPS_DEN = 1.0e-12
const EPS_SEGMENT = 1.0e-8
const DEFAULT_SCAN_POINTS = 1024
const MAX_BISECTION_STEPS = 60
const MAX_SEGMENT_NODES = 2048

# -----------------------------------------------------------------------------
# 基础工具函数

@inline function energy_cutoff(m::Float64, cutoff::Float64)
	return sqrt(m * m + cutoff * cutoff)
end

@inline function internal_momentum(E::Float64, m::Float64)
	val = E * E - m * m
...
@inline function distribution_value(mode::Symbol, sign::Symbol, E::Float64, μ::Float64,
	...
	elseif mode === :fermi
		invT = 1.0 / T
		μ_eff = sign === :plus ? μ : -μ
		return fermi_dirac(E, μ_eff, invT)
	else
...
function real_part_k_positive(sign::Symbol, λ::Float64, k::Float64, m::Float64, mp::Float64,
	...
	@inbounds for idx in eachindex(nodes)
		...
		total += weights[idx] * dist * log(ratio) * invk
	end
	return total
end
