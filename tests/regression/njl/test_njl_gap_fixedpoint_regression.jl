...
const NJL_BASELINE_PATH = joinpath(PROJECT_ROOT, "tests", "baselines", "njl", "baseline_njl_gap_fixedpoints_v1.csv")
...
@testset "NJL gap fixedpoint regression" begin
    rows = _load_rows(NJL_BASELINE_PATH)
    ...
    for row in rows
        ...
        @test isapprox(x[1], row.phi_u; rtol=5e-6, atol=1e-10)
        @test isapprox(x[2], row.phi_d; rtol=5e-6, atol=1e-10)
        @test isapprox(x[3], row.phi_s; rtol=1e-6, atol=1e-10)
        @test isapprox(x[4], row.Phi; rtol=1e-12, atol=1e-12)
        @test isapprox(x[5], row.PhiBar; rtol=1e-12, atol=1e-12)
        @test isapprox(omega, row.omega; rtol=5e-6, atol=1e-10)
    end
end