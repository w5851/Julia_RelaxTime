...
@testset "TransportWorkflow smoke: solver backend bridge (4 fixed points)" begin
    ...
    function _check_bridge_points(points, xi, tau, models_solver)
        for pt in points
            T = pt.T
            mu = pt.mu
            ...
            @testset "bridge point $(point_label)" begin
                res_legacy, res_models = _run_bridge_case(T, mu, xi, tau, models_solver)
    ...
    _check_bridge_points(points, 0.0, tau, models_solver)

    xi_points = ((T=0.90, mu=0.00),)
    _check_bridge_points(xi_points, 0.2, tau, models_solver)
end
