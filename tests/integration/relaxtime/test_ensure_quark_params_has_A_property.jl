...
@testset "Property: Extended QuarkParams with A Field" begin
    ...
    max_examples_main = _ci ? 3 : 20
    max_examples_equiv = _ci ? 2 : 10
    sample_timeout_seconds = _ci ? 20.0 : 90.0

    # Stabilize property-based sampling in CI to avoid long-tail hangs.
    _ci && Random.seed!(0x51A7C2)

    ensure_A_guarded(quark_params, thermo_params) = run_with_timeout_seconds(
        () -> Main.RelaxationTime.ensure_quark_params_has_A(quark_params, thermo_params),
        sample_timeout_seconds,
        "ensure_quark_params_has_A",
    )
    ...
    @testset "QuarkParams without A field - struct input" begin
        ...
        @check max_examples=max_examples_main function property_struct_workflow_support(
            ...
            q_with_A = ensure_A_guarded(q_struct, t_struct)
    ...
    @testset "QuarkParams without A field - NamedTuple input" begin
        ...
        @check max_examples=max_examples_main function property_namedtuple_workflow_support(
            ...
            # Call ensure_quark_params_has_A
            q_with_A = ensure_A_guarded(q_nt, t_nt)
    ...
    @testset "QuarkParams with A field - should preserve it" begin
        ...
        @testset "Struct input with A field" begin
            ...
            q_with_A_again = ensure_A_guarded(q_with_A, t_struct)
        ...
        @testset "NamedTuple input with A field" begin
            ...
            # Call ensure_quark_params_has_A
            q_result = ensure_A_guarded(q_nt, t_nt)
    ...
    @testset "Struct and NamedTuple equivalence" begin
        ...
        @check max_examples=max_examples_equiv function property_struct_namedtuple_equivalence(
            ...
            q_struct_result = ensure_A_guarded(q_struct, t_struct)
            
            # Call with NamedTuple parameters
            q_nt_result = ensure_A_guarded(q_nt, t_nt)
