...
const SCRIPT = joinpath(PROJECT_ROOT, "scripts", "relaxtime", "run_strict_bw_meson_density_scan.jl")
const OUTDIR = joinpath(PROJECT_ROOT, "data", "outputs", "results", "relaxtime", "scan", "test_outputs")
const OUTFILE = joinpath(OUTDIR, "strict_bw_meson_density_scan_smoke.csv")

@testset "strict BW meson density scan smoke" begin
    mkpath(OUTDIR)
