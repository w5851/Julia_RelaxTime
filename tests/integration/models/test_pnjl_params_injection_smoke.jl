...
@testset "Models PNJL params injection (minimal slice)" begin
    ...
    m_unittest = Models.create_model(:PNJL; profile="unittest", physics_profile="default")
    ...
    st = Models.MeanFieldState(SVector{3, Float64}(0.10, 0.10, 0.10); Phi=0.6, PhiBar=0.6)
    T_fm = 0.20
    mu = 0.0

    d_default = Models.number_densities(m_default, st, T_fm, mu; p_num=10, t_num=2, xi=0.0)
    d_unittest = Models.number_densities(m_unittest, st, T_fm, mu; p_num=10, t_num=2, xi=0.0)
