...
@testset "Models phase 0 contract" begin
    ...
    x_state = Models.solve_gap(model, T_fm, mu; p_num=12, t_num=4, xi=0.0, residual_norm_max=1e-5)
    @test x_state isa Models.MeanFieldState

    # Accept scalar μ by contract (treated as (μ, μ, μ)).
    ω = Models.omega(model, x_state, T_fm, mu; p_num=12, t_num=4, xi=0.0)
    @test isfinite(ω)

    comps = Models.omega_components(model, x_state, T_fm, mu; p_num=12, t_num=4, xi=0.0)
    @test all(isfinite, (comps.chi, comps.poly, comps.vac, comps.therm, comps.omega))

    # Contract helpers
    v = Models.state_vector(x_state)
    @test length(v) == 5
    μv = Models.normalize_mu_vec(mu)
    @test length(μv) == 3
end
