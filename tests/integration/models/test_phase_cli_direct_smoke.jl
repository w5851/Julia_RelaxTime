...
function _run_phase_cli_direct(; bracket_mode::String, start::String="low")
    output_dir = mktempdir()
    cmd = `$(Base.julia_cmd()) --project=$(PROJECT_ROOT) $(CLI_SCRIPT) --model_kind=PNJL --T_min=150 --T_max=160 --T_step=10 --rho_min=0.1 --rho_max=0.3 --rho_step=0.1 --output_dir=$(output_dir) --profile=smoke --solver_backend=legacy --cep_strategy=direct --cep_max_bisect_iter=1 --cep_max_refine_level=0 --cep_direct_bracket_mode=$(bracket_mode) --cep_direct_start=$(start) --cep_direct_expand_factor=2.0 --cep_direct_max_expand_steps=4 --cep_direct_fallback_scan=true`
    run(cmd)
...
@testset "Phase CLI direct smoke" begin
    ...
    output_dir, summary_path, summary = _run_phase_cli_direct(bracket_mode="directional", start="low")
...
@testset "Phase CLI direct bracket mode compare" begin
    ...
    _, summary_path_directional, summary_directional = _run_phase_cli_direct(bracket_mode="directional", start="low")
    _, summary_path_scan, summary_scan = _run_phase_cli_direct(bracket_mode="scan", start="low")
