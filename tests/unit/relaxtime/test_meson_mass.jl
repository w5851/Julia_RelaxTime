...
const _RELAXTIME_PATH_MM = normpath(joinpath(@__DIR__, "..", "..", "..", "src", "relaxtime", "RelaxTime.jl"))
if !isdefined(Main, :RelaxTime)
    Base.include(Main, _RELAXTIME_PATH_MM)
end
...
using Main.RelaxTime.MesonMass: ensure_quark_params_has_A, default_meson_mass_guess, meson_mass_equation
const MesonMass = Main.RelaxTime.MesonMass
...
@testset "MesonMass 基础测试" begin
    ...
    @test default_meson_mass_guess(:pi_plus, qp) ≈ qp.m.u + qp.m.d
    @test default_meson_mass_guess(:pi_minus, qp) ≈ qp.m.u + qp.m.d
    @test default_meson_mass_guess(:K, qp) ≈ qp.m.u + qp.m.s
    @test default_meson_mass_guess(:K_plus, qp) ≈ qp.m.u + qp.m.s
    @test default_meson_mass_guess(:K_minus, qp) ≈ qp.m.u + qp.m.s
    ...
    f_pi_plus = meson_mass_equation(:pi_plus, 0.5, 0.0, 0.0, qp, thermo_params, K_coeffs)
    f_k_minus = meson_mass_equation(:K_minus, 0.8, 0.0, 0.0, qp, thermo_params, K_coeffs)
    @test isfinite(real(f_pi_plus)) && isfinite(imag(f_pi_plus))
    @test isfinite(real(f_k_minus)) && isfinite(imag(f_k_minus))
end
