...
@testset "Threshold phase is an analytic limit, independent of counting" begin
    ...
    @test_throws ArgumentError IL.threshold_limit(0.,1.)
    @test_throws ArgumentError IL.threshold_limit(1.,0.)
    @test_throws ArgumentError IL.threshold_limit(1+0.1im,1.)
end

@testset "Gap contour must not cut off a near-Mott real root" begin
    w=IL.gap_window(0.,1.,[1e-8,0.5,1-2e-8])
    @test 0<w.left<1e-8
    @test 1-2e-8<w.right<1
    @test w.left_margin≈1.25e-9
    @test IL.gap_window(0.,1.,Float64[]).left==1e-7
    @test_throws ArgumentError IL.gap_window(0.,1.,[0.])
    @test_throws ArgumentError IL.gap_window(0.,1.,[1.])
end
