...
const IY=CausalGBUInfiniteYield

@testset "Roots counted without a phase or unwrap" begin
    for sign in (-1.,1.)
        roots=IY.bracket_roots(x->sign*(x-0.31)*(x-0.73),0.,1.)
        @test length(roots)==2
        @test roots≈[0.31,0.73] atol=1e-12
    end
    @test isempty(IY.bracket_roots(x->x^2+1,0.,1.))
    @test_throws ArgumentError IY.bracket_roots(identity,1.,0.)
    # Large finite Landau interval must not lose its thermal boundary layer.
    for h in (10.,1e3,1e8)
        @test IY.I.thermal_range(x->exp(-x),0.,h,1.;nodes=96)≈1-exp(-h) atol=1e-12
    end
end
...
@testset "GBU bound/continuum integration-by-parts bookkeeping" begin
    ...
    g(w)=inv(expm1(w/T))
    direct=real(IY.I.C.mapped_integral([threshold,endcut];nodes=96) do w
        g(w)*(1+g(w))/T*IY.O.gbu_weight(delta(w))/pi
    end)-g(threshold)
    derivative=real(IY.I.C.mapped_integral([threshold,endcut];nodes=96) do w
