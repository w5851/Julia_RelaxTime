...
@testset "EffectiveCouplings 模块测试" verbose=true begin
    ...
    @testset "基本功能测试" begin
        @testset "calculate_G_from_A 基本功能" begin
            ...
            A_typical = 1.5  # fm
            m_typical = 0.8  # fm⁻¹
            G_result = calculate_G_from_A(A_typical, m_typical)
            ...
            # 手动验证公式
            expected_G = -N_color / (4.0 * π^2) * (m_typical * A_typical)
    ...
    @testset "物理约束测试" begin
        ...
        @testset "K系数量级检验" begin
            ...
            # K系数应该在合理范围内（与G同量级）
            for (key, val) in pairs(K_coeffs)
                if key == :det_K_plus || key == :det_K_minus
                    # 行列式是 fm^4 量纲，不与 G_fm2 直接同量级比较
                    @test isfinite(val)
                    @test val > 0
                elseif key != :K08_plus && key != :K08_minus
