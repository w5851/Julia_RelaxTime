...
@testset "strict charged phase backend physical diagnostic contract" begin
    ...
    @test occursin("prescription=:ordered_retarded", source)
    @test occursin("threshold_fn", source)
    @test occursin("bound_state_count=q -> 0", source)
    @test occursin("omega_measure=:single_charge_domega_over_pi", source)
