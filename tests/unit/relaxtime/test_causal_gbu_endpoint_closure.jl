...
const EC=CausalGBUEndpointClosure

@testset "Projected exterior bound independent of spectral interpolation" begin
    @test EC.lens_volume(0.,3.)≈4pi*3^3/3
    @test EC.lens_volume(6.,3.)==0
    @test EC.lens_volume(7.,3.)==0
    for q in (0.,0.5,3.,7.,21.)
        r=EC.projected_exterior(q,1.,2.,3.,10.,0.25)
...
@testset "Signed endpoint toy: separate root, continuum and weak eta limit" begin
    ...
    root=EC.toy_exterior_root(a,S,c,K).root
    F(z)=EC.toy_inverse(z,a,S,c,K)
    Fp(z)=4K*c/pi*(-1/(S-z)+1/(S+z)+1/(a-z)-1/(a+z))
    phase(w)=-angle(F(complex(w)))
    W(w)=EC.O.gbu_weight(phase(w))
    dW(w)=2sin(phase(w))^2*(-imag(Fp(complex(w))/F(complex(w))))
    g(w)=EC.O.bose(w,T,0.)
    beta(w)=g(w)*(1+g(w))/T
    integrate(f,es)=EC.A.integrate_panels(f,es;nodes=16,atol=1e-9,max_depth=24)
    ...
    for eta in etas
        ...
        smooth=integrate(w->beta(w)*EC.O.gbu_weight(-angle(F(w+eta*im)))/pi,edges)
...
@testset "Finite thermal edge forces exterior zeros, not UHP instability" begin
    for phi in (0.,0.3,1.),bar in (0.,0.4,1.),T in (0.6,0.9),L in (10.,20.,24.)
        ep=EC.thermal_endpoint(1.6,0.3,2.6,0.01,T,phi,bar,3.,L)
        @test ep.rho_inside_inv_fm2<0
        r=EC.endpoint_obstruction(ep.rho_inside_inv_fm2,ep.lambda_endpoint_inv_fm,1.,-0.1,0.24,T,0.29)
    ...
    @test_throws ArgumentError EC.thermal_endpoint(1.,0.,2.,0.,0.8,1.,1.,3.,3.)
    @test_throws ArgumentError EC.endpoint_obstruction(0.1,5.,1.,0.,0.25,0.8,0.)
end

@testset "Exact signed-band counterexample has a real negative-residue root" begin
    for c in (0.3,0.6,1.),K in (0.4,0.7)
        r=EC.toy_exterior_root(1.,2.,c,K)
        @test r.root>2
        @test abs(EC.toy_inverse(r.root,1.,2.,c,K))<1e-10
        @test r.residue<0
        @test !r.is_positive_physical_bound_state
        @test r.phase_jump≈pi
        @test real(EC.toy_inverse(1im,1.,2.,c,K))>1
        step=min(1e-4,(r.root-2)/10)
        before=-angle(EC.toy_inverse(r.root-step+step*1e-6im,1.,2.,c,K))
        after=-angle(EC.toy_inverse(r.root+step+step*1e-6im,1.,2.,c,K))
        ...
        continuum_units=(EC.O.gbu_weight(-pi)-EC.O.gbu_weight(0.))/pi
        @test continuum_units+1≈0 atol=1e-14
        @test continuum_units+0≈-1 atol=1e-14
    end
    s=EC.stage_status(analytic_structure=false,physical_counting=true,integral_convergence=true)
