...
@testset "Regulator geometry and exact cut restriction" begin
    for (a,b) in ((1.0,1.4),(1.4,1.0),(1.0,1.0)),q in (0.0,0.5,3.0,7.0)
        ...
        for l in (0.2,1.5,4.0,8.0),s in (-1,1),t in (-1,1)
            q==0 && continue
            for (u,v) in RC.centered_intervals(l,q,a,b,L,s,t)
    ...
    @test RC.domain_support(6.0,1.0,1.4,3.0,:two_line)===nothing
    @test RC.merge_intervals([(1.,2.),(-2.,-1.),(1.5,3.)])==[(-2.,-1.),(1.,3.)]
    @test RC.analytic_gaps([(-3.,-2.),(2.,3.)],0.5;upper=5.0)==[(0.,1.5),(2.5,5.)]
    @test_throws ArgumentError RC.domain_support(1.,1.,1.4,3.,:invalid)
    @test_throws ArgumentError RC.domain_support(NaN,1.,1.4,3.,:centered)
    @test_throws ArgumentError RC.centered_intervals(NaN,1.,1.,1.4,3.,1,-1)
    @test_throws ArgumentError RC.centered_cut(0.3,NaN,1.,0.2,1.4,-0.1,0.8,0.3,0.4,3.)
    @test_throws ArgumentError RC.centered_atoms(0.8,1.,0.2,1.4,-0.1,0.8,0.3,0.4,3.,NaN)
end
...
@testset "Full support gaps are counted independently of a normal gap" begin
    ...
    rows=RC.count_support_gaps(bubble,s,cuts;upper=8.0)
    @test length(rows)==2
    ...
    miss=RC.count_support_gaps(even,s,cuts;upper=8.0)
    ...
    boundary=RC.count_support_gaps(near,s,cuts;upper=8.0)
...
@testset "Centered cut Cauchy reconstruction versus independent loop" begin
    bg=(m=(u=1.0,d=1.1,s=1.4),mu=(u=0.15,d=0.1,s=0.05),T=0.8,Phi=0.3,PhiBar=0.4,
        coupling=Dict(c=>0.20 for c in RC.R.CHANNELS),vacuum=3.0)
    for q in (0.0,0.8)
        # q=0 has a hard-cutoff jump; mesh64 gave 5.3--6.0e-4 error.
        # Resolve the endpoint cell, preserving the existing error target.
        low=RC.centered_profile(bg,:K_plus,q;mesh=64,ne=64)
        high=RC.centered_profile(bg,:K_plus,q;mesh=128,ne=64)
        g=RC.centered_atoms(q,1.,0.15,1.4,0.05,0.8,0.3,0.4,3.,24.;np=192,nx=96)
        for z in (0.5+0.6im,3.0+0.6im)
            target=RC.atom_value(g,z).value
            coarse=abs(RC.R.cauchy_transform(low.profile,z)-target)
            fine=abs(RC.R.cauchy_transform(high.profile,z)-target)
            @test fine<coarse
            @test fine<5e-4
        end
        for (left,right) in RC.analytic_gaps(high.support,high.shift)
    ...
    p=RC.R.PiecewiseSpectralFunction([-1.,0.,1.],[0.,1.,0.])
    for gamma in (0.2,0.7,1.0),center in (0.0,0.4)
        integral=0.0
        for (left,right) in ((-1.,0.),(0.,1.))
            xs,ws=RC.R.gauleg(left,right,128)
            integral+=sum(ws[i]*(1-abs(xs[i]))*gamma/(pi*((xs[i]-center)^2+gamma^2)) for i in eachindex(xs))
        end
        @test integral≈imag(RC.R.cauchy_transform(p,center+gamma*im)) atol=1e-13
    end
end

@testset "Independent centered causal loop and contact identity" begin
    for q in (0.0,0.8)
        g=RC.centered_atoms(q,1.,0.2,1.4,-0.1,0.8,0.3,0.4,3.,6.;np=64,nx=48)
        rev=RC.centered_atoms(q,1.4,-0.1,1.,0.2,0.8,0.3,0.4,3.,6.;np=64,nx=48)
        for z in (0.4+0.5im,3.0+0.8im,10.0+2im)
            v=RC.atom_value(g,z)
            @test v.contact_residual<1e-11
            @test abs(v.moment0)<1e-12
            @test v.value≈conj(RC.atom_value(rev,-conj(z)).value) atol=1e-12
        end
        if q==0
            original=RC.R.build_spectral_bubble(q,1.,0.2,1.4,-0.1,0.8;Phi=0.3,PhiBar=0.4,
                vacuum_cutoff_inv_fm=3.,thermal_cutoff_inv_fm=6.,momentum_nodes=64,angle_nodes=48)
            @test RC.atom_value(g,0.4+0.5im).value≈RC.R.spectral_bubble(original,0.1;eta_inv_fm=0.5).value atol=1e-12
            @test g.contact≈original.contact_inv_fm2 atol=1e-12
        end
        for l in (0.2,0.7,3.0,6.0)
            f(a,u,b,v,l,c)=RC.centered_cut(l,q,a,u,b,v,0.8,0.3,0.4,3.;component=c)
            @test f(1.,0.2,1.4,-0.1,l,:thermal)≈-f(1.4,-0.1,1.,0.2,-l,:thermal) atol=1e-12
            @test f(1.,0.,1.,0.,0.2,:vacuum)==0
        end
        @test abs(RC.centered_cut(0.3,q,1.,0.2,1.4,-0.1,0.8,0.3,0.4,6.;component=:thermal))<1e-12
    end
end
