...
@testset "MesonPropagator 模块测试" verbose=true begin
    ...
    @testset "辅助函数测试" begin
        ...
        @testset "calculate_coupling_matrix 函数" begin
            ...
            # 手动验证M矩阵元素
            M00_expected = K_coeffs.K0_plus - (4.0 / 3.0) * (Π_uu_test + 2.0 * Π_ss_test) * K_coeffs.det_K_plus
            M08_expected = K_coeffs.K08_plus + (4.0 / 3.0) * sqrt(2.0) * (Π_uu_test - Π_ss_test) * K_coeffs.det_K_plus
            M88_expected = K_coeffs.K8_plus - (4.0 / 3.0) * (2.0 * Π_uu_test + Π_ss_test) * K_coeffs.det_K_plus
    ...
    @testset "一般介子传播子测试（meson_propagator_simple）" begin
        ...
        @testset "通道-K系数映射验证" begin
            # 验证π使用K123_plus
            D_pi = meson_propagator_simple(:pi, K_coeffs, Π_uu_P)
            D_pi_manual = (2.0 * K_coeffs.K123_plus) / (1.0 - 4.0 * K_coeffs.K123_plus * Π_uu_P)
            @test abs(D_pi - D_pi_manual) < 1e-15
            
            # 验证K使用K4567_plus
            D_K = meson_propagator_simple(:K, K_coeffs, Π_us_P)
            D_K_manual = (2.0 * K_coeffs.K4567_plus) / (1.0 - 4.0 * K_coeffs.K4567_plus * Π_us_P)
            @test abs(D_K - D_K_manual) < 1e-15
            
            # 验证σ_π使用K123_minus
            D_sigma_pi = meson_propagator_simple(:sigma_pi, K_coeffs, Π_uu_S)
            D_sigma_pi_manual = (2.0 * K_coeffs.K123_minus) / (1.0 - 4.0 * K_coeffs.K123_minus * Π_uu_S)
            @test abs(D_sigma_pi - D_sigma_pi_manual) < 1e-15
            
            # 验证σ_K使用K4567_minus
            D_sigma_K = meson_propagator_simple(:sigma_K, K_coeffs, Π_us_S)
            D_sigma_K_manual = (2.0 * K_coeffs.K4567_minus) / (1.0 - 4.0 * K_coeffs.K4567_minus * Π_us_S)
