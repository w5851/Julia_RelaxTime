...
const PR=CausalGBUPereiraReference
...
const PCB=Main.CausalSpectralBubble
...
@testset "Pereira: occupation reduction and exact pair/scattering algebra" begin
    for x in (-1000.,-10.,-0.2,0.,0.2,10.,1000.)
        ...
        @test PR.occupation(x,1.,1.)≈fd atol=2e-16
        @test PR.occupation(x,0.,0.)≈(x>=0 ? exp(-3x)/(1+exp(-3x)) : 1/(1+exp(3x))) atol=2e-16
    end
    for (phi,bar) in ((1.,1.),(0.3,0.4)),mu in (-0.3,0.2)
        n=PR.occupations(1.1,mu,0.8,phi,bar)
    ...
    for c in (0.,1.)
        ...
        ref=sum(PR.pair_scattering(E1,E2,z,n1,n2,c))
    ...
    @test PR.quadratic_intervals(1.,0.,-1.,-2.,2.)==[(-1.,1.)]
    @test PR.quadratic_intervals(-1.,0.,1.,-2.,2.)==[(-2.,-1.),(1.,2.)]
    @test PR.quadratic_intervals(0.,1.,0.,-2.,2.)==[(-2.,-0.)]
    @test isempty(PR.quadratic_intervals(1.,0.,1.,-2.,2.))
end

@testset "Pereira: exact lens volume and same-domain single-line terms" begin
    for q in (0.,0.01,0.8,3.,5.9,6.,6.1)
        L=3.
        r=PR.cylindrical_reference(q,1.,0.2,1.4,-0.1,0.8,L,[0.4+0.6im];
                                  component=:vacuum,nz=64,ny=64)
        expected=q<2L ? pi*(2L-q)^2*(4L+q)/12 : 0.
        @test r.volume≈expected atol=1e-11
        @test r.A1≈PR.vacuum_A(q,1.,L) atol=2e-11
        @test r.A2≈PR.vacuum_A(q,1.4,L) atol=2e-11
        @test !r.production_authorized
    end
    r=PR.cylindrical_reference(0.8,1.,0.2,2.7,0.5,0.8,3.,[0.4+0.6im];component=:vacuum)
    @test r.A1≈PR.vacuum_A(0.8,1.,3.) atol=2e-11
    # The inherited single-line term is not a q-independent equilibrium A.
    @test abs(r.A1-PR.vacuum_A(0.,1.,3.))>1.
    @test_throws ArgumentError PR.cylindrical_reference(-1.,1.,0.,1.,0.,0.8,3.,[1im])
    @test_throws ArgumentError PR.cylindrical_reference(1.,1.,0.,1.,0.,0.8,3.,[1.])
    @test_throws ArgumentError PR.vacuum_A(1.,0.,3.)
end
...
@testset "Pereira: independent complex loop, P/S contact, and PNJL extension" begin
    ...
    for q in (0.,0.8,3.),(phi,bar) in ((1.,1.),(0.3,0.4))
        ...
        r=PR.cylindrical_reference(q,a,u,b,v,T,L,zs;Phi=phi,PhiBar=bar,nz=80,ny=80)
        rev=PR.cylindrical_reference(q,b,v,a,u,T,L,-conj.(zs);Phi=phi,PhiBar=bar,nz=80,ny=80)
        @test r.b0≈conj.(rev.b0) atol=2e-10
        for channel in (:P,:S)
            g=PCB.build_spectral_bubble(q,a,u,b,v,T;Phi=phi,PhiBar=bar,channel=channel,
            ...
            for (i,z) in enumerate(zs)
                d=PCB.spectral_bubble(g,real(z)-u+v;eta_inv_fm=imag(z))
...
@testset "Pereira: C18/C19 retarded cuts, support, and thermal decomposition" begin
    for q in (0.,0.8,3.,6.),l in (-4.0,-0.2,0.,0.2,4.0),component in (:full,:vacuum,:thermal)
        ...
        r=PR.reference_cut(l,q,a,u,b,v,T,L;Phi=phi,PhiBar=bar,component=component)
    ...
    for q in (0.,0.8),l in (0.2,3.0)
        f(c)=PR.reference_cut(l,q,1.,0.2,1.4,-0.1,0.8,3.;component=c).imaginary
        @test f(:full)≈f(:vacuum)+f(:thermal) atol=1e-13
        plus=PR.reference_cut(3.,q,1.,0.,1.4,0.,0.8,3.;component=:vacuum)
        minus=PR.reference_cut(-3.,q,1.,0.,1.4,0.,0.8,3.;component=:vacuum)
        @test plus.pair>0 && minus.pair<0 # mass-i0 would instead be even
        @test plus.pair≈-minus.pair atol=1e-13
    end
    @test PR.reference_cut(0.,0.,1.,0.,1.,0.,0.8,3.).static_degeneracy_unresolved
    @test_throws ArgumentError PR.reference_cut(NaN,1.,1.,0.,1.,0.,0.8,3.)
end
...
const PA=CausalGBUPereiraAudit
@testset "Pereira: runner validation and synthetic adapters" begin
    @test_throws ArgumentError PA.main(nodes=15)
    @test_throws ArgumentError PA.main(qs=(NaN,))
    @test_throws ArgumentError PA.main(qs=())
    @test_throws ErrorException PA.R.start_output(_PEREIRA_ROOT)
    bg=(m=(u=1.,d=1.4,s=1.8),mu=(u=0.2,d=-0.1,s=0.),
        T=0.8,vacuum=3.)
    zs=[0.4+0.6im,3.2+0.8im]
    combined=PA.reference(bg,:pi_plus,0.8,zs,0.3,0.4,3.,48)
    full=PR.cylindrical_reference(0.8,1.,0.2,1.4,-0.1,0.8,3.,zs;
    ...
    for l in (-3.,-0.2,0.2,3.)
        c=PA.cut(bg,:pi_plus,0.8,l,0.3,0.4,3.,64)
        f=PR.reference_cut(l,0.8,1.,0.2,1.4,-0.1,0.8,3.;Phi=0.3,PhiBar=0.4)
