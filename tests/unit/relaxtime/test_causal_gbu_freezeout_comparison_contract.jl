...
@testset "GBU freezeout comparison algebra" begin
    ...
    out = FGC.aggregate_components(rows; qmax=2.0, nq=8)
    @test length(out) == 4
    ...
    @test_throws ErrorException FGC.aggregate_components(rows[1:end-1]; qmax=2.0, nq=8)
    ...
    rr = FGC.ratio_comparison(direct, reference)
    @test length(rr) == 3
