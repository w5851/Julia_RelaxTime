...
const SF=CausalGBUSourceFunctional
...
@testset "Fixed projector determinant differentiates to two projected lines" begin
    ...
    for keep in (falses(4),trues(4),Bool[1,1,0,0],Bool[1,0,1,0])
        f=j->SF.projected_action(d,V,keep,j)
        exact=SF.projected_hessian(d,V,keep)
        @test ForwardDiff.derivative(j->ForwardDiff.derivative(f,j),0.)≈exact atol=1e-13
        @test exact>=0
    end
    keep=Bool[1,1,0,0]
    two=SF.projected_hessian(d,V,keep)
    ...
    f=j->SF.projected_action(d,V,keep,j)+SF.projected_action(d.+0.7,V,trues(4),j)-
        SF.projected_action(d,V,trues(4),j)
    @test ForwardDiff.derivative(j->ForwardDiff.derivative(f,j),0.)≈
        two+SF.projected_hessian(d.+0.7,V,trues(4))-SF.projected_hessian(d,V,trues(4)) atol=1e-13
    @test_throws ArgumentError SF.projected_action([-1.],ones(1,1),[true],0.)
end

@testset "Complex color determinant reproduces PNJL algebra, not positivity" begin
    for (phi,bar) in ((0.,0.),(0.3,0.4),(1.,1.)),x in (-2.,0.,3.)
        ell=SF.color_eigenvalues(phi,bar)
        y=exp(-x)
        @test prod(1 .+ell*y)≈1+3phi*y+3bar*y^2+y^3 rtol=2e-13
        @test sum(ell*y./(1 .+ell*y))/3≈SF.P.occupation(x,phi,bar) atol=1e-13
        @test prod(1 .+y./ell)≈1+3bar*y+3phi*y^2+y^3 rtol=2e-13
    end
    @test any(abs.(abs.(SF.color_eigenvalues(0.3,0.4)).-1).>0.01)
    @test_throws ArgumentError SF.color_eigenvalues(-0.1,0.3)
end
...
@testset "Dirac source trace equals the on-shell A/B numerator" begin
    ...
    for p in ((0.,0.,0.),(0.3,0.2,0.7)),r in ((0.2,0.,-0.1),(0.,0.,0.)),
        ...
        @test SF.spin_trace(p,r,a,b,s,t,ch)≈formula atol=2e-15
        @test SF.spin_trace(p,r,a,b,s,t,ch)>=-1e-15
    end
end
...
@testset "Static source trace-log and ordered dynamic Hessian" begin
    for ch in (:P,:S),component in (:vacuum,:thermal),phi in (0.3,1.)
        ...
        e1=sqrt(sum(abs2,p)+a^2);e2=sqrt(sum(abs2,r)+b^2)
        direct=3sum(SF.spin_trace(p,r,a,b,s,t,ch)*
            SF.response_quotient(s*e1-u,t*e2-v,0.,T,phi,0.4,component)
            for s in (-1,1),t in (-1,1))
        f(j)=SF.block_action(p,r,a,u,b,v,T,phi,0.4,j,component;channel=ch)
        h=0.002
        d1=(f(h)+f(-h)-2f(0.))/h^2
    ...
    for phi in (0.3,1.),tc in (2.5,6.)
        a,u,T=1.2,0.1,0.8
        response=SF.source_response(0.,a,u,a,u,T,2.5,tc,[0.];Phi=phi,PhiBar=0.4,nodes=32)
        state=SF.B.static_flavor(a,u,T,phi,0.4,2.5,tc;nodes=96)
    ...
    @test_throws ArgumentError SF.source_response(0.,1.,1.1,1.,0.,0.8,2.,4.,[0.])
    @test_throws ArgumentError SF.source_response(0.,1.,0.,1.,0.,0.8,2.,4.,[1.])
    # Exercise the integrated finite-difference assembly, not only a local block.
    for q in (0.,0.6),ch in (:P,:S)
        rs=SF.source_response(q,1.2,0.1,1.7,0.25,0.8,2.5,4.,[0.,1+0.8im];
        ...
        g=SF.R.build_spectral_bubble(q,1.2,0.1,1.7,0.25,0.8;Phi=0.3,PhiBar=0.4,
            channel=ch,vacuum_cutoff_inv_fm=2.5,thermal_cutoff_inv_fm=4.,
            momentum_nodes=48,angle_nodes=48)
        @test SF.R.spectral_bubble(g,1.;eta_inv_fm=0.8).value≈rs.pi_inv_fm2[2] atol=1e-8
    end
end
