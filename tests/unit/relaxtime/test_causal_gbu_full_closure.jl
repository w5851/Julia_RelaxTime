...
const testcausalgbufullclosure_FCO=CausalGBUCoordinateOracles
const testcausalgbufullclosure_FCG=CausalGBUGapCompleteness
include(joinpath(_FC_ROOT,"scripts","analysis","relaxtime","summarize_causal_gbu_full_closure.jl"))
const testcausalgbufullclosure_FCS=CausalGBUFullClosureSummary
...
@testset "Closure reducer rejects missing evidence and false passes" begin
    ...
    @test testcausalgbufullclosure_FCS.check_case(row,profiles,meshes,gaps,cuts)
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(row,profiles[2:end],meshes,gaps,cuts)
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(row,profiles,meshes,gaps[2:end],cuts)
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(row,profiles,meshes,gaps,cuts[2:end])
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(merge(row,(passed=false,)),profiles,meshes,gaps,cuts)
    broken=[merge(m,(support_excess_width_inv_fm=1e-6,)) for m in meshes]
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(row,profiles,broken,gaps,cuts)
    ...
    @test !testcausalgbufullclosure_FCS.check_case(failed,slow,meshes,gaps,cuts)
    @test_throws ErrorException testcausalgbufullclosure_FCS.check_case(merge(failed,(passed=true,)),slow,meshes,gaps,cuts)
end
...
@testset "Same regulator in independent radial coordinates" begin
    ...
    for q in (0.,0.5,7.),regulator in (:two_line,:centered)
        radial=testcausalgbufullclosure_FCO.radial_loop(q,a,u,b,v,T,phi,phibar,L,H,zs,regulator;np=64,nr=64)
        if regulator===:two_line
            grid=testcausalgbufullclosure_FCO.R.build_spectral_bubble(q,a,u,b,v,T;Phi=phi,PhiBar=phibar,
                vacuum_cutoff_inv_fm=L,thermal_cutoff_inv_fm=H,momentum_nodes=96,angle_nodes=64)
            values=[testcausalgbufullclosure_FCO.R.spectral_bubble(grid,real(z)-u+v;eta_inv_fm=imag(z)).value for z in zs]
            contact=grid.contact_inv_fm2
        else
            grid=testcausalgbufullclosure_FCO.C.centered_atoms(q,a,u,b,v,T,phi,phibar,L,H;np=96,nx=64)
            values=[testcausalgbufullclosure_FCO.C.atom_value(grid,z).value for z in zs]
    ...
    @test isempty(testcausalgbufullclosure_FCO.radial_domain(6.,3.,:two_line))
    @test_throws ArgumentError testcausalgbufullclosure_FCO.radial_domain(0.,3.,:unknown)
    @test_throws ArgumentError testcausalgbufullclosure_FCO.radial_loop(1.,a,u,b,v,T,phi,phibar,L,H,[1.],:centered)
end
...
@testset "Geometric endpoints cannot leak into interpolant gaps" begin
    ...
    bg=(m=(u=1.6519019722802555,d=1.649895213451757,s=2.5),
        ...
        Phi=0.3,PhiBar=0.4,coupling=Dict(c=>0.2 for c in testcausalgbufullclosure_FCO.R.CHANNELS),vacuum=3.0)
    for ch in (:pi_plus,:pi_minus),q in (0.0,1.0,3.0,6.0),reg in (:two_line,:centered)
        a,b=testcausalgbufullclosure_FCO.R.charged_rpa_spec(ch).pair
        m1,m2,u,v=bg.m[a],bg.m[b],bg.mu[a],bg.mu[b]
        cuts=testcausalgbufullclosure_FCO.C.support_intervals(q,m1,m2,3.0,24.0,reg)
        if reg===:two_line
            g=testcausalgbufullclosure_FCO.R.build_spectral_bubble(q,m1,u,m2,v,bg.T;Phi=bg.Phi,PhiBar=bg.PhiBar,
                vacuum_cutoff_inv_fm=3.0,thermal_cutoff_inv_fm=24.0,momentum_nodes=8,angle_nodes=8)
            p=testcausalgbufullclosure_FCO.R.build_bubble_dispersion(g;segment_nodes=16)
        else
            p=testcausalgbufullclosure_FCO.C.centered_profile(bg,ch,q;mesh=16,ne=32).profile
        end
        @test testcausalgbufullclosure_FCG.support_geometry_check(testcausalgbufullclosure_FCG.signed_cells(p),cuts).passed
        outer=last(cuts)[2]
        for l in (outer,nextfloat(outer),-outer,prevfloat(-outer))
            @test imag(testcausalgbufullclosure_FCO.R.cauchy_transform(p,l))==0
        end
    end
    # The thermal tail is ~1e-24, still inside support and nonzero.
    cut=testcausalgbufullclosure_FCO.C.centered_cut(47.0,3.0,bg.m.u,bg.mu.u,bg.m.d,bg.mu.d,0.4,
...
@testset "Signed-cell enclosures include zeros, gaps and infinite tails" begin
    p=testcausalgbufullclosure_FCG.R.PiecewiseSpectralFunction([1.,2.,3.],[0.,1.,0.])
    cells=testcausalgbufullclosure_FCG.signed_cells(p)
    @test length(cells)==2
    ...
    @test_throws ArgumentError testcausalgbufullclosure_FCG.inverse_enclosure(cells,1.,1.5,2.5)
    ...
    near=1.0-1e-7
    K=1/(4real(testcausalgbufullclosure_FCG.R.cauchy_transform(p,near)))
    narrow=testcausalgbufullclosure_FCG.count_all_real_gaps(p,K,0.)
    @test narrow.passed && narrow.count==1
    @test first(narrow.rows).margin_inv_fm<1e-7
    @test testcausalgbufullclosure_FCG.support_geometry_check(cells,[(1.,3.)]).passed
    @test !testcausalgbufullclosure_FCG.support_geometry_check(cells,[(1.01,3.)]).passed
    @test !testcausalgbufullclosure_FCG.support_geometry_check(cells,[(0.9,3.)]).passed
    # Tiny but nonzero spectra are not thresholded into fake analytic gaps.
    tiny=testcausalgbufullclosure_FCG.R.PiecewiseSpectralFunction([-1.,0.,1.],[0.,1e-200,0.])
    @test testcausalgbufullclosure_FCG.profile_support(testcausalgbufullclosure_FCG.signed_cells(tiny))==[(-1.,1.)]
    signed=testcausalgbufullclosure_FCG.R.PiecewiseSpectralFunction([-2.,-1.,1.,2.],[0.,-1.,1.,0.])
    @test length(testcausalgbufullclosure_FCG.signed_cells(signed))==4
    @test testcausalgbufullclosure_FCG.profile_support(testcausalgbufullclosure_FCG.signed_cells(signed))==[(-2.,2.)]
    rounded=testcausalgbufullclosure_FCG.R.PiecewiseSpectralFunction([-2.,-1.,1.,2.],[0.,-1e-30,1.,0.])
    triangles=testcausalgbufullclosure_FCG.signed_cells(rounded)
    @test length(triangles)==4
    for w in (-4.,4.)
        @test sum(testcausalgbufullclosure_FCG.cell_real(c,w) for c in triangles)/pi≈real(testcausalgbufullclosure_FCG.R.cauchy_transform(rounded,w)) atol=1e-14
    end
    # Even root: winding finds multiplicity two; a simple-root scan MUST fail.
    even=testcausalgbufullclosure_FCG.R.PiecewiseSpectralFunction([-3.,-2.,-1.,1.,2.,3.],[0.,-1.,0.,0.,1.,0.])
    coupling=1/(4real(testcausalgbufullclosure_FCG.R.cauchy_transform(even,0.)))
    audit_even=testcausalgbufullclosure_FCG.count_all_real_gaps(even,coupling,0.)
