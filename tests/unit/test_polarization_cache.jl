...
@testset "极化函数缓存测试" begin
    ...
    @testset "性能对比" begin
        ...
        # 第二次调用（缓存）
        result2 = nothing
        time_cache = @elapsed begin
            for i in 1:1000
                result2 = polarization_aniso_cached(channel, k0, k_norm, m1, m2, μ1, μ2, T, Φ, Φbar, ξ, A1, A2, num_s)
            end
        end
        
        speedup = time_compute / (time_cache / 1000)
        println("性能测试：")
        println("  计算时间: $(time_compute * 1000) ms")
        println("  缓存时间: $(time_cache / 1000 * 1e6) μs (平均)")
        println("  加速比: $(speedup)x")
        
        # 缓存应该更快（实际加速比取决于极化函数计算复杂度）
        @test speedup > 1.0  # 确保缓存比计算快
        @test result1 == result2  # 确保结果一致
    end
