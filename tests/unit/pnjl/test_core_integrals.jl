...
@testset "cached_nodes" begin
    @testset "默认节点数" begin
        p_mesh, cosθ_mesh, coefficients = Integrals.cached_nodes(Integrals.DEFAULT_MOMENTUM_COUNT, Integrals.DEFAULT_THETA_COUNT)
        @test size(p_mesh) == (Integrals.DEFAULT_MOMENTUM_COUNT, Integrals.DEFAULT_THETA_COUNT)
        @test size(cosθ_mesh) == (Integrals.DEFAULT_MOMENTUM_COUNT, Integrals.DEFAULT_THETA_COUNT)
        @test size(coefficients) == (Integrals.DEFAULT_MOMENTUM_COUNT, Integrals.DEFAULT_THETA_COUNT)
    end
    
    @testset "自定义节点数" begin
        p_num, t_num = 24, 8
        p_mesh, cosθ_mesh, coefficients = Integrals.cached_nodes(p_num, t_num)
    ...
    @testset "缓存一致性" begin
        nodes1 = Integrals.cached_nodes(48, 12)
        nodes2 = Integrals.cached_nodes(48, 12)
    ...
    @testset "节点范围" begin
        p_mesh, cosθ_mesh, _ = Integrals.cached_nodes(48, 12)
...
@testset "vacuum_integral" begin
    @testset "正质量" begin
        mass = 1.5  # fm⁻¹, ~300 MeV
        result = Integrals.vacuum_integral(mass)
    ...
    @testset "质量单调性" begin
        # 质量越大，真空项越大
        m1 = Integrals.vacuum_integral(0.5)
        m2 = Integrals.vacuum_integral(1.0)
        m3 = Integrals.vacuum_integral(1.5)
        @test m1 < m2 < m3
    end
    
    @testset "负质量处理" begin
        # 应该使用绝对值
        result_pos = Integrals.vacuum_integral(1.0)
        result_neg = Integrals.vacuum_integral(-1.0)
...
@testset "calculate_energy_sum" begin
    ...
    result = Integrals.calculate_energy_sum(masses)
    @test isfinite(result)
...
@testset "calculate_energy_isotropic" begin
    mass = 1.5
    p = 1.0
    E = Integrals.calculate_energy_isotropic(mass, p)
...
@testset "calculate_energy_anisotropic" begin
    ...
    @testset "xi=0 退化为各向同性" begin
        E_iso = Integrals.calculate_energy_isotropic(mass, p)
        E_aniso = Integrals.calculate_energy_anisotropic(mass, p, 0.0, 0.5)
    ...
    @testset "xi>0 增加能量" begin
        cosθ = 0.5
        E_iso = Integrals.calculate_energy_isotropic(mass, p)
        E_aniso = Integrals.calculate_energy_anisotropic(mass, p, 0.5, cosθ)
        @test E_aniso > E_iso
    end
    
    @testset "cosθ=0 时无各向异性修正" begin
        E_iso = Integrals.calculate_energy_isotropic(mass, p)
        E_aniso = Integrals.calculate_energy_anisotropic(mass, p, 0.5, 0.0)
        @test isapprox(E_iso, E_aniso; rtol=1e-10)
    end
end
...
@testset "calculate_log_sum" begin
    ...
    p_nodes, cosθ_nodes, coefficients = Integrals.cached_nodes(24, 6)
    Φ = 0.3
    Φ̄ = 0.3
    ...
    result = Integrals.calculate_log_sum(masses, p_nodes, cosθ_nodes, coefficients, Φ, Φ̄, mu_vec, T_fm, xi)
    @test isfinite(result)
...
@testset "calculate_log_sum_derivatives" begin
    if isdefined(Integrals, :calculate_log_sum_derivatives)
        ...
        @testset "与 calculate_log_sum 一致" begin
            log_sum_direct = Integrals.calculate_log_sum(masses, p_nodes, cosθ_nodes, coefficients, Φ, Φ̄, mu_vec, T_fm, xi)
