...
@testset "entrypoints" begin
    ...
    @testset "薄转发接口存在" begin
        ...
        @test isdefined(Models, :run_meson_mass_path_scan)
        @test isdefined(Models, :run_freezeout_meson_mass_scan)
        @test isdefined(Models, :run_isentropic_meson_mass_scan)
        @test isdefined(Models, :run_crossover_meson_density_scan)
