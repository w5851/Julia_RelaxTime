下面给你一个 **清晰、标准、可直接用于实际计算（尤其是夸克散射、热核反应率、输运方程等）** 的 QFT 中 **(2\to2)** 反应率计算方法。分为三层：从最基础的 S-矩阵 → 微分截面 → 反应率（含热平均）。
...
[
\frac{d\sigma}{d\Omega} = \frac{1}{64\pi^2 s}, \frac{|\mathbf p_f|}{|\mathbf p_i|}, \overline{|\mathcal M|^2}.
]
...
极其重要：
这个 (v_{\rm rel}) 正是保证整个反应率表达式 Lorentz-invariant 的关键。
...
# ✅ 4. QFT 中更原始、更严谨的形式（Lorentz 不变 measure）
...
其中 ( dσ/dΩ^* ) 完全由 ( |\mathcal M|^2 ) 决定，包括介子极化函数、传播子角度依赖等。
