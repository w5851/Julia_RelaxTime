...
二阶导数 ∂²φ_u/∂T² 存在多个符号变化（拐点）。从高温向低温扫描：
- 第一个 - → +（~200 MeV）：大峰（手征 crossover）的拐点
- 后续的 + → - 和 - → +：小峰（退禁闭引起）的拐点
...
**定义**：找 ∂²φ/∂T² = 0 的位置（峰值右侧的拐点）

**算法**：
1. 从高温向低温扫描，计算 ∂²φ/∂T²
2. 找所有 - → + 的符号变化（对应峰值右侧的拐点）
3. 过滤掉 |dφ/dT| 过小的虚假拐点（阈值：最大值的 15%）
4. 选择 |dφ/dT| 最大的有效拐点（对应真正的 crossover）
5. 使用**二分法**细化拐点位置
...
- 峰值位置：∂²φ/∂T² = 0（拐点）
- 峰值右侧：∂²φ/∂T² > 0（凹）
- 从高温向低温扫描时，第一个有效的 - → + 变化对应最大峰的拐点

**虚假拐点过滤**：
- 在高温区域（T > 240 MeV），Polyakov loop Φ 接近饱和值 1
- 此时 dΦ/dT ≈ 0，可能产生虚假的符号变化
- 通过检查 |dφ/dT| 是否足够大来过滤这些虚假拐点
