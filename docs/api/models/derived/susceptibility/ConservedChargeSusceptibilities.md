...
- 显式 `derivative_backend=:forwarddiff` 已下线，会抛出迁移错误

如果超出当前支持范围，接口会抛出参数错误，而不是静默退化。

`derivative_backend` 支持：

- `:auto`：默认策略。纯 B/Q/S 单方向走单变量 TaylorDiff fast path；mixed BQS 组合走内部 multivariate Taylor jet。
...
- `:forwarddiff`：退休的 legacy 选项；调用会抛出迁移错误。
...
这些关键字只影响 TaylorDiff / MixedTaylorJet backend；旧 ForwardDiff backend 已下线。
...
- 如果你直接调用 `flavor_pressure_derivatives`，它现在支持 `derivative_backend=:auto|:taylordiff|:mixedjet`；其中 `:auto` 与 `:mixedjet` 都会走 TaylorDiff 方向组合，`:forwarddiff` 会抛出迁移错误
