...
- `GaussLegendre` —— 提供分段、端点聚簇和高斯-勒让德节点生成（内部工具）
...
`i0` 方向构造当前项目 `e^{-iωt}` retarded 边界的复数值。它不是
`Complex(Re(B0), -Im(B0))` 的全局翻号；后者只在部分 cut 区域成立。它是
...
B0(λ::Float64, k::Float64, m1::Float64, μ1::Float64,
   ...
   Φ::Float64=0.0, Φbar::Float64=0.0) -> NTuple{2, Float64}
```
...
- 实部在 A±B 根处分段；光滑/单侧/双侧区间分别使用预计算 Gauss--Legendre、幂次聚簇或 DE 映射。`k=0` 主值分支使用奇点减法与解析对数项。
...
- `B0` 当前没有公开 `rtol`/`atol` 参数；高精度审计应在代表点上与独立高节点或隔离 benchmark oracle 比较。
- 对同一组参数重复调用时，应复用外层状态，并用节点/分段收敛证据确认误差，而不是假定固定规则自带容差估计。
...
- 对于较大的 `k`，被积函数中的对数项可能出现数值负值；实现中使用绝对值、显式根分段和端点聚簇确保稳定。
...
- **如何验证结果收敛？** 在少量代表性参数上提高独立固定节点数或使用 `benchmark/` 环境中的外部 oracle，并检查结果随节点/分段加密稳定。
