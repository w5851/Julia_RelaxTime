...
`cpvi` 的定位是历史诊断节点生成器，而不是严格的
`epsilon -> 0+` 主值实现：它固定使用 `0.001h` 的有限间隙，因此结果必须随
节点数和间隙做收敛检查。`OneLoopIntegrals.B0` 的 `k=0` 分支另行使用奇点减法
与解析对数端点项计算主值实部；charged phase 审计中的
`B0_pv_cut` 在此基础上再显式拼接 retarded cut 边界值。生产路径不得把
`cpvi` 的有限间隙直接当作已经收敛的 PV 结果。
