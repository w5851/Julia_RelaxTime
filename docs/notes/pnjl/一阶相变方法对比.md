...
> 文档版本：2025-12-30
> 相关代码：
> - T-ρ 扫描 + Maxwell 构造：`src/pnjl/analysis/PhaseTransition.jl`
...
```julia
# 方法 1：T-ρ + Maxwell
result_maxwell = maxwell_construction(mu_vals, rho_vals)
...
# 对比
@assert abs(result_maxwell.mu_transition - transition.mu_c) < 0.1
```
...
- 相变分析模块：`src/pnjl/analysis/PhaseTransition.jl`（S 形检测 + Maxwell 构造）
- 双分支扫描：`src/pnjl/scans/DualBranchScan.jl`
- 双分支策略文档：`docs/notes/pnjl/一阶相变双分支扫描策略.md`
- 参考数据：`data/reference/pnjl/boundary.csv`
