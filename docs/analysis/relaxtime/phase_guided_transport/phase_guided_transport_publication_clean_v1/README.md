...
- 本包生成图：72 张 PNG（6 个 panel × 12 个 observable），见 `figures/plot_manifest.json`
- observable 合同：核心比值 `eta_over_s, zeta_over_s, sigma_over_T`；新增字段 `tau_u, tau_d, tau_s, tau_ubar, tau_dbar, tau_sbar, eta, sigma, zeta`。
...
- 论文显示 marker：24 条（12 个 observable × 2 个已确认区间），均为区间中点的 display-only proxy。
...
- `tables/publication_clean_points.csv`：12 种论文展示 observable 的长表，含 raw/clean/status。
- `tables/curve_index.csv`：216 条 panel/series/observable 曲线的覆盖和替换计数。
- `tables/claim_ledger.csv`：证据强度、范围限制和未声明事项。
- `figures/plot_manifest.json` 与 `figures/**.png`：同构的 72 张显示候选图，未声明为 manuscript-ready。
