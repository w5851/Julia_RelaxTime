...
- The finite-q/q=0 comparison and historical freezeout overlay remain in the
  ...
  Their CSV/JSON source payloads remain there, while the selected PNG display
  payloads and figure manifests are stored under this package's `figures/`
  directory. Their manifests all set `production_authorized=false`; the
  historical overlay is explicitly a context-only, unmatched projection.
...
The detailed light inventory covers 33 evidence files (1,654,579 bytes):
...
The selected commit scope contains 24 payload files and 1,441,104 bytes
(18 finite-q/q=0 comparison payloads and 6 historical-overlay payloads). The
two untracked source snapshots, generated duplicate plot files, and the
historical SVG are excluded from that scope. The complete main-tree analysis root currently contains 3,583
files and 33,925,663 bytes. Its aggregate inventory hash is recorded in
...
`D:/w/jrt-ord` is retained. Its corresponding payloads are byte-equivalent to
the selected main-tree evidence, but the figure payloads remain under the old
`data/outputs/figures` layout there; the main-tree relocation therefore changes
inventory path labels without changing the PNG bytes. Its larger root also
contains historical/repeated diagnostics that are not present in main. No
worktree or diagnostic file is deleted by this change.
