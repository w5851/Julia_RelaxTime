...
这是已从 `freezeout_kminus_piminus_mu_pi_100` 正式图目录迁出的历史 plot-review evidence，归入 `docs/analysis`。它用于人工检查 workflow 与 digitized target 的趋势和差异，不是 external validation gate、regression truth source 或 `strict` 论文图。
