...
本 v4 版本为 `visualization-only`：所有有限/converged Maxwell boundary 行（包括自动 geometry/interpolation 未闭合行）统一以蓝色绘制，隐藏状态颜色和高于 CEP 的灰色响应峰。它不生成合成数值行、不改变源曲线、不跨越显式三角化 gap，也不把未闭合证书升级为闭合。
