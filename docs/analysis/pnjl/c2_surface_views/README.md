...
The seven cases share the C2/v5 baseline source run `31862752226`, but each preserves its own semantic verdict, tables, decision record, and manifest. v6 additionally records the endpoint expansion numerical/replay provenance. They are `diagnostic_only` and do not promote unresolved geometry, CEP midpoints, endpoint overlays, or visual closure to phase-reference truth.
