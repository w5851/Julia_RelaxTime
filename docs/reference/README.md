...
公式块采用 GitHub 兼容的 `$$ ... $$` 分隔符，行内公式采用 `$ ... $` 分隔符。Markdown 标准本身不定义数学语法，因此预览器必须启用 KaTeX 或 MathJax 才会排版公式；若只显示原始 TeX 文本，通常是预览器的数学分隔符或插件配置不兼容。VS Code 的 Markdown 预览还取决于当前版本和扩展设置；确认其数学渲染已启用，或使用支持 KaTeX/MathJax 的 Markdown 扩展。公式源仍然保留在文档中，可在 GitHub 或支持 KaTeX/MathJax 的预览器中查看排版结果。
