...
```text
eB + magnetic config
-> MFIR vacuum: zero-field 3D cutoff + finite Hurwitz-zeta correction
-> Landau-level thermal matter term
...
$$
\Omega_f^{\mathrm{vac}}(B)=\Omega_f^{\mathrm{vac}}(0;\Lambda,M_f)
 -\frac{N_c(|q_f|eB)^2}{2\pi^2}
\left[\zeta'(-1,x)-\frac{x^2-x}{2}\ln x+\frac{x^2}{4}\right],
\qquad x=\frac{M_f^2}{2|q_f|eB}.
$$

其中零场真空项使用共享 `PNJLCore.vacuum_integral_with_cutoff` 的三动量截断，
有限场项使用 `MagneticIntegrals.omega_magnetic_mfir`。热 flavor 项仍为

$$
\Omega_{f,\mathrm{thermal}}=-T\frac{|q_f|eB}{2\pi}\sum_n\alpha_n
\int\frac{dp_z}{2\pi}(Z_f^++Z_f^-),
$$

旧完整 Landau 真空项仍可通过 `MagneticConfig(route=:landau_legacy)` 复现：

$$
\Omega_{f,\mathrm{legacy}}^0=-N_c\frac{|q_f|eB}{2\pi}\sum_n\alpha_n
...
当前生产源码/TOML 使用 `a=0.0108805`；高雪艳博士论文表 5-1 的
`0.108805` 已确认是十倍排版/记录错误。Ferreira 2014/2018 以及
旧 Fortran、`pnjl_mag` 也使用 `a=0.0108805`。这不是单位转换差异：内部
`eB_fm2` 与 `Lambda_QCD/hbarc` 在 `zeta` 中使用同一单位，转换因子会抵消。
MFIR 路线已接入运行时配置；在 `pnjl_cep` 外部源、commit、单位和输出合同固定前，
磁场外部数值验证仍保持 `diagnostic_only`，不把旧 Fortran 或 `pnjl_mag` 直接晋升为 acceptance。
...
正则化不同，且作者反馈指出 smooth-Landau 截断存在问题。贺伟博硕士论文给出的
...
运行时构造器已接入 `magnetic_default.toml`；后续外部测试集优先转向待提供源码/commit
的 `pnjl_cep`，不是继续扩展 smooth-Landau replay。
...
来源核验范围：高雪艳博士论文《强相互作用物质相变与重子数涨落的研究》第 2.2 节印刷页 21--24、第五章第 5.1 节印刷页 65--68；MFIR 细节以贺伟博硕士论文第 4 章印刷页 33--39 为实现参考，并以 `pnjl_cep` 的磁场无关正规化约定作为待固定的外部核对路线。原文有符号/排版不一致（式(2-63)重复 `phi_u`，式(5-10)导数符号与式(2-65)冲突，式(5-11)使用带符号 `q_f B`），所以本页不把这些冲突字符升级为无条件代码合同。IMC 参数的正式决策为 `a=0.0108805`；Julia 的 `MeV^2 -> fm^-2` 转换在 `zeta=eB/Lambda_QCD^2` 中抵消，未发现隐藏十倍补偿。运行时配置已接入，详见 [`magnetic_imc_parameter_provenance_v1.md`](../analysis/historical/legacy/legacy_extraction_v1/magnetic_imc_parameter_provenance_v1.md)。
...
- **证据**：magnetic unit、thermodynamics unit、fixed-point regression，以及低节点 `solve_magnetic_gap` stationarity/branch probe；固定点证据覆盖固定 `x_state` 的内核/回归，不等于默认高节点磁场 equilibrium 或全分支全集已验收。Fortran 与 `pnjl_mag` 的外部交叉核对台账目前仅是路线诊断；`pnjl_cep` 的仓库、commit、公式和输出合同尚未提供，因此外部 acceptance gate 仍阻塞。
...
- **数值边界**：默认 MFIR 不用 Landau 求和表示真空项；`n_max`、`p_z_max` 只控制热 Landau 项，`zeta_num` 控制 MFIR 特殊函数的数值积分。`route=:landau_legacy` 下的 `smooth_cutoff`、真空 `n_max` 和 pz Gauss 节点仅为历史诊断。论文的低温占据 `n_max` 估计不等同于真空项的求和上限；这些数值参数在 `pnjl_cep` acceptance 前仍需收敛审计。
