...
```text
eB + magnetic config
-> Landau levels and current diagnostic vacuum route
...
当前源码/TOML 的旧值 `a=0.108805` 来自高雪艳博士论文表 5-1；Ferreira 2014/2018 以及
旧 Fortran、`pnjl_mag` 使用 `a=0.0108805`。本项目已决定正式 profile 采用
`a=0.0108805`；当前旧值尚待运行时配置修复。这不是单位转换差异：内部
`eB_fm2` 与 `Lambda_QCD/hbarc` 在 `zeta` 中使用同一单位，转换因子会抵消。
在配置接线和正则化路线完成前，磁场外部数值验证保持 `diagnostic_only`，不生成
...
运行时构造器尚未接入 `magnetic_default.toml`；后续外部测试集优先转向待提供源码的
`pnjl_cep`，不是继续扩展 smooth-Landau replay。
...
来源核验范围：高雪艳博士论文《强相互作用物质相变与重子数涨落的研究》第 2.2 节印刷页 21--24、第五章第 5.1 节印刷页 65--68。原文有符号/排版不一致（式(2-63)重复 `phi_u`，式(5-10)导数符号与式(2-65)冲突，式(5-11)使用带符号 `q_f B`），所以本页不把这些冲突字符升级为无条件代码合同。IMC 参数的正式决策为 `a=0.0108805`；当前 Julia 配置仍为旧值 `a=0.108805`，而 Ferreira 2014/2018、旧 Fortran 和 `pnjl_mag` 为 `a=0.0108805`；Julia 的 `MeV^2 -> fm^-2` 转换在 `zeta=eB/Lambda_QCD^2` 中抵消，未发现隐藏十倍补偿。运行时修复尚未完成，详见 [`magnetic_imc_parameter_provenance_v1.md`](../analysis/historical/legacy/legacy_extraction_v1/magnetic_imc_parameter_provenance_v1.md)。
...
- **数值边界**：当前 `n_max`、`p_z_max` 和 smooth cutoff 只描述诊断实现，尚未获得论文/外部 source-gated acceptance；在 MFIR 或 `pnjl_cep` 路线确认前，不把它们称为生产正则化合同。论文的低温占据 `n_max` 估计不等同于真空项的求和上限；`p_num/pz_max/rtol` 等为实现参数，不能从论文公式推断。
