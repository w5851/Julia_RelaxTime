...
- 数密度 $\rho_i$ 和 $\rho_j$ 由下式给出：
  $$
  \rho_i = d_q \frac{1}{(2\pi)^2} \int_0^\Lambda p_i^2 dp_i \int_{-1}^1 d\cos\theta_i \ f_i(p_i,\cos\theta_i)
  $$
  类似地计算 $\rho_j$。
...
- 数密度 \( \rho_i \) 和 \( \rho_j \) 简化为：
  $$
  \rho_i = \frac{d_q}{2\pi^2} \int_0^\Lambda p_i^2 dp_i \int_{0}^1 d\cos\theta_i \ f_i(p_i,\cos\theta_i)
  $$
  同理计算 \( \rho_j \)。
