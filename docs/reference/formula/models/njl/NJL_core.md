...
### 📌 适用范围与来源说明
- 本节主公式（2-12）至（2-34）为**三味NJL（u,d,s）**推导结果。
- 论文2.1节未单独给出两味巨热力学势的完整推导式；以下两味表达式为从三味结果做标准退化后得到，供实现与对照使用。
- 为避免与原文编号混淆，以下使用“补-”编号。

### 🔁 三味到两味的退化规则
1. 味求和范围由 \(\sum_{i=u,d,s}\) 改为 \(\sum_{i=u,d}\)。
2. 去除六费米子耦合项 \(-4K\phi_u\phi_d\phi_s\)（两味模型通常不含 't Hooft 六费米子项）。
3. 使用两味参数集 \((m,\Lambda,G)\) 重新标定，不与三味参数混用。
...
\[
\begin{aligned}
\Omega_{\text{2f}} = & -2N_c \sum_{i=u,d} \int \frac{d^3p}{(2\pi)^3} \frac{p^2}{3E_i} \left[ n(E_i - \mu_i) + \bar{n}(E_i + \mu_i) \right] \\
& -2N_c \sum_{i=u,d} \int^{\Lambda} \frac{d^3p}{(2\pi)^3} E_i \\
& + 2G\left(\phi_u^2+\phi_d^2\right) + C
\end{aligned}
\quad (\text{补-1})
\]

其中：
- \(E_i=\sqrt{M_i^2+\mathbf{p}^2}\)
- \(n(E_i-\mu_i)=1/(e^{(E_i-\mu_i)/T}+1)\)
- \(\bar n(E_i+\mu_i)=1/(e^{(E_i+\mu_i)/T}+1)\)
...
在同位旋对称近似下可取 \(m_u=m_d\), \(\phi_u=\phi_d\)。

### ⚙️ 两味NJL参数表（常用基准集）

> 说明：两味参数并非唯一，不同文献会采用不同 \(\Lambda\) 与 \(G\Lambda^2\) 组合；建议在数值任务中固定一个参数集并全流程复用。

| 参数集 | \(m_u=m_d\) (MeV) | \(\Lambda\) (MeV) | \(G\Lambda^2\) | 备注 |
|---|---:|---:|---:|---|
| Set-2f-A | 5.5 | 631 | 2.14 | 常见于两味NJL综述与教学基准 |
| Set-2f-B | 5.5 | 590 | 2.19 | 另一常用等效标定 |

可由 \(G=(G\Lambda^2)/\Lambda^2\) 还原有量纲耦合常数（单位 MeV\(^{-2}\)）。

### 🔗 与本文三味主线的对应关系
- 对应式（2-32）：去除 \(s\) 味并去掉 \(K\) 项，得到补-1。
- 对应式（2-13）：去除味混合项 \(2K\phi_j\phi_k\)，得到补-2。
- 两味参数集仅用于两味计算，不应与上文三味参数表（文献[141]）交叉代入。
