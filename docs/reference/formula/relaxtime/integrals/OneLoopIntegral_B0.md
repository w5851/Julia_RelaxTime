...
```math
\tilde{B}_0^{\pm}(\lambda, k, m_1, m_2, \mu_1, \mu_2, T, \varphi) = 2 \int_{m_1}^{\sqrt{m_1^2 + \lambda^2}} p \, n^\pm (\pm E, \mu)\int_{-1}^{+1}  \frac{1}{\lambda^2 + 2 \lambda E + 2pk x - k^2 + m_1^2 - m_2^2} d x d E
```

### 3.2.1 NJL vs PNJL：分布函数导致的等价改写（关键差异来源）

本项目里 B0 的“\(\pm\)”（或代码里的 `pm=±1`）语义，**本质上是对能量自变量取 \(\pm E\)**。这里的 \(\pm E\) 来自于松原频率求和/留数分解时，对传播子分母 \((i\omega+\mu)^2-E^2\) 的裂项与两支极点（\(\pm E\)），**并不是因为先做了某个分布函数恒等式变换才“产生”了负能量**；反粒子的物理能量依然是正的。

化学势口径（工程约定）：在本项目中把输入参数 \(\mu\) 视为“净夸克数（或重子数）化学势”的非负量，因此对外接口约定 \(\mu\ge 0\)。对于反夸克分布，PNJL 的公式形式本身已经把符号体现在指数的 \((E+\mu)/T\) 中，因此调用 `antiquark_distribution(E, μ, ...)` 时 **直接传入同一个非负 \(\mu\)** 即可（不要在调用侧手动取反）。

数值实现的关键点：虽然 \(pm=-1\) 在形式上对应 \(f^{+}(-E,\mu)\)，但实现中**尽量只在正能量 \(E>0\)** 上评估分布函数，以避免直接计算负能量时出现指数溢出；因此会使用严格恒等式把 `1-f^-` 改写为 `f^+(-E,μ)`（或反向改写），这一步的动机是**数值稳定/避免溢出**，而不是 \(\pm E\) 出现的根源。更完整推导可参考：`docs\reference\domain-knowledge\relaxtime\松原频率求和应用于A与B0的具体计算.md`。

值得注意的是，NJL下自由费米子气体中没有对夸克和反夸克做公式形式上的区分，而PNJL中则通过公式形式上的区分使得化学势在代入公式计算时总是正的，因此PNJL中f(E,μ)=1-f(-E,μ)的恒等式看上去和NJL中的f(E+μ)=1-f(-E-μ)有所不同，但实际上是等价的。

另外需要注意避坑：NJL 模型下 `f(E-μ)-f(-E-μ)=f(E-μ)+f(E+μ)-1` **并不是净占据数**（而是“总占据数 - 1”）。真正的净占据数是“夸克分布 − 反夸克分布”，即 `f(E-μ)-f(E+μ)`。
...
定义
```math
f_{\rm NJL}^{+}(E,\mu)=\frac{1}{e^{\beta(E-\mu)}+1},\qquad
f_{\rm NJL}^{-}(E,\mu)=\frac{1}{e^{\beta(E+\mu)}+1}
```
则有基本恒等式
```math
f_{\rm NJL}^{+}(E,\mu)+f_{\rm NJL}^{-}(-E,\mu)=1
\quad\Rightarrow\quad
1-f_{\rm NJL}^{-}(E,\mu)=f_{\rm NJL}^{+}(-E,\mu)
```
也就是说，在 B0 的 Matsubara 求和/留数分解里出现的 `1 - f^{-}`（泡利阻塞因子）可以等价写成 `f^{+}(-E,\mu)`。

#### (2) PNJL（含 Polyakov loop 的有效分布）

PNJL 下的有效分布（记作 \(f_{\Phi}^{\pm}\)）见 [PNJL_夸克有效分布函数.md](../../models/pnjl/distribution/PNJL_夸克有效分布函数.md)。同样满足
```math
...
**这就是 C++/Julia 实现里最关键的“口径差异来源”**：
在 B0 的四项分解中，本来可以写成 `f^{+}(E,\mu)` 与 `1-f^{-}(E,\mu)` 的组合；
但实现时常把 `1-f^{-}(E,\mu)` **等价替换**为 `f^{+}(-E,\mu)`，从而统一写成 `f^{+}(pm\,E,\mu)`（`pm=±1`）。

因此：
- `pm = +1` 代表 `f^{+}(E,\mu)`；
- `pm = -1` 代表 `f^{+}(-E,\mu)`，它等价于 `1-f^{-}(E,\mu)`。

这也解释了为何代码里会出现“把 `1-f^-` 写成 `f^+`”的表象：它不是近似，而是利用了上面的恒等式。
其中k是外部动量
```math
...
关于当前项目 `B_0` 与文献常用 `I_2` 的严格归一化对应，以及它如何同极化函数/有效耦合/极点方程同时闭合，见独立说明页：
