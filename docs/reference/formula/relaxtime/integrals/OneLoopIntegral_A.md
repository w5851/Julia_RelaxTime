...
```
## 3. 额外说明
- PNJL模型下，参数需要加上Φ和Φbar；f(E-μ)和f(E+μ)分别表示夸克和反夸克的有效分布函数，具体形式见 [PNJL_夸克有效分布函数.md](../../models/pnjl/distribution/PNJL_夸克有效分布函数.md)（更详细推导见 [PNJL_夸克有效分布函数_补充详细说明.md](../../models/pnjl/distribution/PNJL_夸克有效分布函数_补充详细说明.md)）。
- 在实现层面，常用恒等式 `1 - f⁻(E, μ) = f⁺(-E, μ)` 把泡利阻塞因子写成“负能量的 f⁺”，从而统一用 `f⁺(pm·E, μ)`（`pm=±1`）表达。
