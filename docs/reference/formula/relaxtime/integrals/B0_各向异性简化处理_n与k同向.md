...
### 关于 “pm=-1 / 负能量” 分支的实现口径（重要）

在 B0 的松原频率求和/留数分解中，\(\pm\) 的本质语义是对能量自变量取 \(\pm E\)（来自传播子分母 \((i\omega+\mu)^2-E^2\) 的裂项与两支极点），而不是“反粒子物理能量必须为负”。

但在 PNJL 的数值实现中，为避免直接在 \(E<0\) 下评估分布函数引发指数中间量溢出，通常把
\[
f^{+}(-E,\mu)\;\text{严格改写为}\;1-f^{-}(E,\mu),\qquad (E>0)
\]
从而始终只在正能量上计算分布函数。

对各向异性一阶修正而言，我们只保留 \(\delta f\)（对 \(\xi\) 的线性项），常数 1 不产生修正，因此
\[
\delta f^{+}(-E,\mu)=\delta(1-f^{-}(E,\mu))=-\delta f^{-}(E,\mu).
\]
这会导致在最终的 B0 组合（例如 \(B_0=\tilde B_0^+ - \tilde B_0^- + \cdots\)）中，\(pm=-1\) 对应的两项在 \(\xi\) 一阶修正的线性组合里出现额外的符号翻转（“负负得正”）。
