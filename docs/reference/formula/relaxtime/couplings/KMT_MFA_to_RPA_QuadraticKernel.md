...
状态：Phase 0/1 的公式合同与诊断实现。本文不是完整 RPA 推导，也不授予 production 资格。
...
令 `s=+1`（P）或 `s=-1`（S），并写 `phi_u=u`、`phi_d=d`、`phi_s=sigma_s`。Phase 1 使用下列候选公式：
...
`src/models/pnjl_physics/PNJLCore.jl` 当前的标量平均场质量方程已经允许 `phi_u`、`phi_d`、`phi_s` 分开出现，但在实际平衡状态和旧传播子接口中，u/d 介子耦合仍被合并。Phase 1 不改变这些上游方程，只把三味凝聚作为输入生成一个可审计的完整候选核。
...
完整 `0/3/8` 结构可追溯到 KMT 六费米项的 Hartree 收缩和三味 RPA 组织。项目调研中使用的主要线索是 Rehberg、Klevansky、Hüfner（Phys. Rev. C 53, 410, DOI: [10.1103/PhysRevC.53.410](https://doi.org/10.1103/PhysRevC.53.410)）和 Mei 等（Phys. Rev. D 107, 074018, [arXiv:2212.04778](https://arxiv.org/abs/2212.04778)）。Mei 等给出了由 `sigma_u-sigma_d` 产生的 `0/3/8` 非对角项；其凝聚和 KMT 符号需先转换到本项目表 2 的约定。

Phase 1 将这些式子标为“候选公式”，因为在接入极化矩阵前仍需逐项核对：
...
只有完成上述核对、并在同位旋对称极限和一个外部文献固定点通过验证后，才允许进入后续完整 RPA 或生产路线。
