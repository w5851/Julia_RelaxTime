...
| `scripts/relaxtime/run_combined_meson_density_scan.jl` | 组合式介子数密度扫描入口，按 scan path × density regime 输出同一批点的 stable / strict BW / phase-shift 多口径 CSV、README、SVG 与图像 manifest | `domain-candidate` | 当前实现 `--path tmu` 与 smoke-only `--path trho_asymmetric`；`tmu` 支持多个固定 `mu_q` 路径与 `phase_display=fold_0_pi` FIG3-like 诊断，`trho_asymmetric` 复用 `FixedAsymmetricRho` equilibrium source 并输出约束诊断字段，默认每个温度内按 `--trho-reverse-rho=true` 从高 rho 到低 rho 连续传递 equilibrium seed；`--density-policy x_min_cut --bose-x-min <x>` 只作用于 BU/GBU phase-shift regimes，stable/BW 保持 strict Bose-domain diagnostics；正式数据默认写入 `data/outputs/results/...`，图像默认写入对应 `data/outputs/figures/...`，后续可扩展 freezeout/crossover/external path |
