...
| `scripts/relaxtime/run_combined_meson_density_scan.jl` | 组合式介子数密度扫描入口，按 scan path × density regime 输出同一批点的 stable / strict BW / phase-shift 多口径 CSV、README、SVG 与图像 manifest | `domain-candidate` | 当前实现 `--path tmu` 与 `--path trho_asymmetric`；`tmu` 支持多个固定 `mu_q` 路径与 `phase_display=fold_0_pi` FIG3-like 诊断，`trho_asymmetric` 复用 `FixedAsymmetricRho` equilibrium source 并输出约束诊断字段，默认 `asym_ud_ratio_target=0.876` 是 `rho_Q/rho_B=0.4` 口径在 `asym_s_target=0` 下的历史近似实现；默认每个温度内按 `--trho-reverse-rho=true` 从高 rho 到低 rho 连续传递 equilibrium seed；`--density-policy x_min_cut --bose-x-min <x>` 只作用于 BU/GBU phase-shift regimes，stable/BW 保持 strict Bose-domain diagnostics；正式产物必须先通过 convergence gate 与 production audit，正式数据默认写入 `data/outputs/results/...`，图像默认写入对应 `data/outputs/figures/...`，后续可扩展 freezeout/crossover/external path |
| `scripts/relaxtime/run_phase_shift_meson_thermo_scan.jl` | canonical `mu_B=0` phase-shift 介子热力学温扫，输出 meson/QP/LD pressure、total EOS、effective config 与 run manifest | `domain-candidate` | 默认 `phase_shift_current + pi/sigma_pi + xi=0`；已有 integration、fixedpoint、canonical-path 与 plot-review regression；当前尚无 external validation gate，不进入 Phase A 核心稳定白名单 |
| `scripts/analysis/relaxtime/render_combined_meson_density_fig3_like.py` | 从组合式介子数密度 CSV 渲染 heatmap PNG/SVG 并更新图像 manifest | `analysis-helper` | 只负责绘图，不重新计算物理量；正式计算仍以 `run_combined_meson_density_scan.jl` 为入口；默认横轴为 `muq_MeV`，`trho_asymmetric` 正式图应使用 `--x-field rho_target --x-label "rho/rho0" --x-unit ""`；跨数量级比值图可用 `--color-scale log` |
