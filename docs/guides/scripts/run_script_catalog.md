...
| `scripts/analysis/relaxtime/render_combined_meson_density_fig3_like.py` | 从组合式介子数密度 CSV 渲染 heatmap PNG/SVG 并更新图像 manifest | `analysis-helper` | 只负责绘图，不重新计算物理量；正式计算仍以 `run_combined_meson_density_scan.jl` 为入口；默认横轴为 `muq_MeV`，`trho_asymmetric` 正式图应使用 `--x-field rho_target --x-label "rho/rho0" --x-unit ""` |
