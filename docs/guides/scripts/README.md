...
- [scripts/relaxtime/run_phase_guided_transport_scan.jl](../../../scripts/relaxtime/run_phase_guided_transport_scan.jl)
  ...
  - 当前 phase-guided transport formal raw case：
    - 高 xi 分辨率、onshell-kernel、p128 validated-anchored 积分精度：`first_canonical_v2_p128_xi001_onshellkernel_validated_anchored_prod_v2`
      ...
      - v4 is a formal publication layout only: `figure_status=author_accepted_formal_layout`, while `numerical_status=diagnostic_only`, `solver_called=false`, and `production_write=false`. The immutable v3 package remains the intermediate parent snapshot; it is not the current publication layer.
      - the explicit acceptance and current-layer pointer are `docs/analysis/relaxtime/phase_guided_transport/publication_clean_formalization_v1.json` and `docs/analysis/relaxtime/phase_guided_transport/publication_clean_current.json`.
