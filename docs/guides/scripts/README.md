...
- [scripts/relaxtime/run_phase_guided_transport_scan.jl](../../../scripts/relaxtime/run_phase_guided_transport_scan.jl)
  ...
  - 旧 `first_canonical_v1` 已从仓库数据树移除。它缺少 `validated_anchored` sigma-cache policy、显式高精度 tau/sigma 积分参数、channel diagnostics 和全网格 convergence gate；历史对照请通过 git history / PR #122 查看，局部 `xi` 结构的 production-grade 解读应使用 p128 production-grade case。
