...
- [scripts/relaxtime/run_phase_guided_transport_scan.jl](../../../scripts/relaxtime/run_phase_guided_transport_scan.jl)
  ...
  - 当前 phase-guided transport formal raw case：
    - 高 xi 分辨率、onshell-kernel、p128 validated-anchored 积分精度：`first_canonical_v2_p128_xi001_onshellkernel_validated_anchored_prod_v2`
      ...
      - rebuild the solver-free analysis layer and mirror it with hash verification using `python scripts/analysis/relaxtime/build_phase_guided_publication_clean_v1.py` followed by `python scripts/analysis/relaxtime/sync_phase_guided_publication_clean_figure_layer.py`; tau/absolute-transport plots retain current `prod_v2` raw values unless a separate tau-specific recipe is reviewed
