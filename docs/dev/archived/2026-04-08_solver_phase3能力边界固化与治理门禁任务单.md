---
title: Solver Phase 3 能力边界固化与治理门禁任务单（稳定契约与防回耦）
archived: true
original: docs/dev/active/2026-04-08_solver_phase3能力边界固化与治理门禁任务单.md
...
- [x] 风险 A：清理兼容字段导致历史调用断裂。
  - 回滚点：恢复适配层，不回滚核心契约分层。

- [x] 风险 B：依赖门禁过严导致正常开发受阻。
...
- [x] Commit 1：契约文档 + 类型导出整理。
- [x] Commit 2：phase/scans/workflows 适配层改造。
- [x] Commit 3：契约稳定性测试 + 依赖门禁。
...
- [x] 2026-04-09：完成任务单口径收口（用于归档）。
  - 补齐验证计划 smoke 三项勾选、风险项状态与建议提交粒度 1/2/3 勾选。
  - 文档结论更新为“已完成（含文档收口）”。
