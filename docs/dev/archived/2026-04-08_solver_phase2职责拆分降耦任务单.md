---
title: Solver Phase 2 职责拆分与降耦任务单（编排/后处理/诊断解耦）
archived: true
original: docs/dev/active/2026-04-08_solver_phase2职责拆分降耦任务单.md
...
- [x] 抽取统一热力学后处理模块，消除重复实现。
- [x] 引入运行时配置类型，替代关键路径 `Dict{Symbol,Any}` 管道。
- [x] 诊断结构类型化并提供兼容映射层。
- [x] 固化 attempt/governance 的策略边界，mode executor 仅保留残差与约束求解职责。

### 2.2 非范围（本期不做）

- [x] 不在本期完成所有历史字段清理（先保兼容，Phase 3 再收口）。
- [x] 不在本期调整 physics 公式或数值策略阈值（避免与架构改动耦合）。
- [x] 不在本期删除 `ConstraintSolverFixed*.jl`（只做职责瘦身和调用对齐）。
...
- [x] B3 改造 `src/models/solver/Solver.jl`
...
- [x] `tests/integration/relaxtime/test_transport_workflow_smoke.jl`（归类为建议附加点测，非 Phase 2 DoD 硬门禁）
- [x] `tests/integration` 中涉及 `PMPhase` 与 `Tmu/Trho` 的 smoke 用例（按仓库现有入口挑选，已由现有 integration smoke 覆盖）
...
- [x] 2026-04-09：完成任务单口径收口（用于归档）。
  - 2.1 范围总项、B3 与建议附加点测条目已统一为“完成/已覆盖”状态，避免与执行记录冲突。
  - 文档结论由“进行中”更新为“已完成（含文档收口）”。
