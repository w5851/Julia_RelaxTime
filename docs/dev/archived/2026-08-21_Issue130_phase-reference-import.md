...
- PR #257 已将统一只读索引 `docs/analysis/pnjl/phase_surface_series/` 合并到 `main`，merge SHA 为 `ac3ce42f06870f4e1eda8ea67922b96ce0f51501`。
...
- 该 namespace 不改变 strict/derived/render 的数值语义，不写入新的 `data/reference/**`，不切换 runtime consumer，不启动 solver 或 RS transport。下一步是独立的只读 runtime consumer compatibility audit；本任务仍未完成 phase-reference promotion。
