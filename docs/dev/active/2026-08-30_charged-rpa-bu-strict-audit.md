...
当前状态：研究生产入口与配置冻结线验收完成，作者已审核图像；工程合入记录见第22.25节及PR310。
...
已将最新链路保存为26ddf1a8，并修复PR307的五条审查意见：重复claim/theme ID、
公式控制字符、旧diagnostic后端丢弃GL权重、标题范围不符；解析测试另确认旧
祖先多除1/T，已回移最新分支的量纲修正。后者不在无限热生产产额调用链中。
PR307相关217/217、PR308相关237/237、PR309相关266/266通过，均已推送新head。
依赖合并只遇到台账断言和相同量纲修正旁的注释冲突，按各分支身份及最新实现
解决，没有替换已验收无限热核。五条PR307 review thread已附说明并解决。

完整core首次暴露共享Main中的测试短别名A与旧导入冲突；改成逐文件唯一别名。
随后修复Julia隐式乘法3B/6R的漏改。失败保留为测试集成证据，不改物理容差。
相关恢复测试478/478和204/204通过；最终33fc215b的本地完整core已通过
12951/12951（8m22.8s），该head的14项GitHub检查全部通过。
所有data/outputs、tmp、work均未入库，v2数值产物及其manifest不重写。

#### 22.25.1 工程合入与研究产物交接

仓库实际合并策略使用squash。每层合并后核对main与该PR最终head的完整文件树
相等，再解决下一层因祖先身份变化产生的冲突；不能用旧祖先覆盖后续实现。
PR307合入83aaa917，PR308合入2fbf49ca；相应最终head的全部检查均已通过。
PR308的822060f5和PR309的658bfaf7仅修复合并历史，文件树分别与此前已验收
的4dca6cf6和5fd87881相同，不改数值算法。

本节在PR310最终合入前记录验收事实。后续层的最终head、CI及合并SHA以
[PR309](https://github.com/w5851/Julia_RelaxTime/pull/309)和
[PR310](https://github.com/w5851/Julia_RelaxTime/pull/310)的合并记录与收尾评论为准，
不是新的数值运行。各层只在最终head检查通过后合入，不使用保护绕过。

再次独立核验v2的146个output_hashes全部匹配，manifest仍为第22.24节的
2c180374b66ce5a0563a44be1f697341cdbb90910bc700e1be0ce6b926be2078。
源码合入与产物保留分别记账：v2原source_snapshot仍是该次计算的复现依据；
后续review、测试隔离和文档修正不能改写其来源身份。

交接范围为固定quark-only BQS背景上的无限热GBU研究部分产额。旧默认、
