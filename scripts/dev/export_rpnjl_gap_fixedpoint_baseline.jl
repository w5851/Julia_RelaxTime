...
const DEFAULT_OUTPUT = joinpath(PROJECT_ROOT, "tests", "baselines", "rpnjl", "baseline_rpnjl_gap_fixedpoints_v1.csv")
...
const GAP_POINTS = [
    ...
    (label="transition_mu0", T_MeV=150.0, mu_MeV=0.0),
    (label="transition_mu100", T_MeV=150.0, mu_MeV=100.0),
    (label="dense_mu200_highT", T_MeV=180.0, mu_MeV=200.0),
]
...
function parse_args(args::Vector{String})
    ...
    while i <= length(args)
        ...
        elseif arg in ("-h", "--help")
            println("Usage: julia --project=. scripts/dev/export_rpnjl_gap_fixedpoint_baseline.jl [--output <path>]")
            exit(0)
        else
...
function main(args::Vector{String})
    ...
    println("RPNJL gap fixedpoint baseline written to: " * output)
