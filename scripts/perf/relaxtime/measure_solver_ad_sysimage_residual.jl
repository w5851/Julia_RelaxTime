...
function _count_focus_lines(path::String)
    isfile(path) || return 0
    re = r"ForwardDiff|DifferentiationInterface|NLSolversBase|typeof\(Models\.solve\)|Models\._solve_constraint_fixedmu|Models\.Conditions"
    count = 0
...
const SUMMARY_PATH = joinpath(TRACE_DIR, "solver_ad_$(LABEL)_summary.json")
const NO_SYS_TRACE = joinpath(TRACE_DIR, "solver_ad_$(LABEL)_no_sys.jl")
const WITH_SYS_TRACE = joinpath(TRACE_DIR, "solver_ad_$(LABEL)_with_sys.jl")
...
println("[solver-ad-sysimage] running no-sys workload...")
no_sys_ms = _run_timed(no_sys_cmd)

println("[solver-ad-sysimage] running with-sys workload...")
...
println("[solver-ad-sysimage] summary")
