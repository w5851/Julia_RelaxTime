...
raw"""
按散射过程扫描散射矩阵元 |M|^2 随 s 的变化（固定 T, μ_B, ξ）。

要求：对不同散射过程，必须先计算物理阈值并保证取到的 (s,t) 在物理区间内。
这里对每个 s 计算 t 的物理边界 (t_min,t_max)，并取
  t_mid = (t_min + t_max)/2
来定义一条“物理可行”的 |M|^2(s) 曲线。

输出 CSV（每行一个 s 点）：
  process, s_th, sqrt_s_th, sqrt_s, s, t_min, t_max, t_mid, M2

示例：
  julia --project=. scripts/relaxtime/scan_scattering_amplitude_vs_s_mid_t.jl \
    ...
    --out data/processed/results/relaxtime/M2_vs_s_mid_t_T150_muB800.csv --overwrite
"""
...
function print_usage()
    println("Usage: julia --project=. scripts/relaxtime/scan_scattering_amplitude_vs_s_mid_t.jl [options]\n")
    println("Options:")
...
function parse_args(args::Vector{String})::Options
    opts = Dict{Symbol,Any}(
        ...
        :n_threshold => 240,
        :out => joinpath("data", "processed", "results", "relaxtime", "M2_vs_s_mid_t.csv"),
        :overwrite => false,
    )
...
function run_scan(opts::Options)
    ...
    io = open(opts.out, "w")
    try
        println(io, "# schema: M2_vs_s_mid_t_v1")
        ...
        println(io, "process,T_MeV,muB_MeV,xi,s_th,sqrt_s_th_MeV,delta_sqrt_MeV,s,sqrt_s_MeV,t_min,t_max,t_mid,M2")
        ...
        for process in opts.processes
            ...
            δs = logspace(max(opts.delta_sqrt_mev_min, 1e-12), max(opts.delta_sqrt_mev_max, opts.delta_sqrt_mev_min*10), opts.n_threshold)

            @printf("process=%s  sqrt_s_th=%.3f MeV  points=%d\n", string(process), sqrt_s_th_mev, length(δs))
            ...
            for δ_mev in δs
                ...
                t_mid = (isfinite(tb.t_min) && isfinite(tb.t_max)) ? 0.5*(tb.t_min + tb.t_max) : NaN

                M2 = (isfinite(t_mid)) ? safe_M2(process, s, t_mid, params) : NaN
