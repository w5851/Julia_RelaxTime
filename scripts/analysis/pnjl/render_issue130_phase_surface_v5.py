#!/usr/bin/env python3
"""Render the Issue #130 phase surfaces as the v5 display candidate.

v5 is a display-only style variant.  It reads the immutable derived surface
tables, keeps the existing endpoint and low-temperature display policies, and
emits a versioned PNG/SVG package.  It does not call the PNJL solver, modify
strict/derived rows, or replace the public phase-reference alias.
"""
...
DEFAULT_OUTPUT_ROOT = Path("data/outputs/figures/pnjl/phase_reference/issue130_phase_reference_v5")
DISPLAY_EVIDENCE_ROOT = Path(
    "docs/analysis/pnjl/phase_reference/issue130_display_candidates_v1/v5"
)
STYLE_PROFILE = "balanced_cartesian_equal_aspect_lit_v5"
CEP_LINE_COLOR = "#c2185b"
CEP_LINE_WIDTH = 4.0
CEP_HALO_WIDTH = 7.0
MU_ZERO_COLOR = "#161616"
MU_ZERO_LINE_WIDTH = 2.4
AXIS_COLOR = "#242424"
...
def render_style_metadata() -> dict[str, Any]:
    return {
        ...
        "view": {"elevation_deg": 30.0, "azimuth_deg": -45.0},
        "box_aspect": [1.0, 1.0, 1.0],
        "surface_shading": "neutral_directional_light_and_fine_edge_contrast_v5",
...
def _draw_inner_axes_v5(axis: Any) -> None:
    """Draw the physical Cartesian triad with separated manual labels."""
    ...
    axis_style = {"color": AXIS_COLOR, "linewidth": 1.6, "alpha": 0.95}
    tick_fontsize = 8.5
    ...
    for mu in mu_ticks:
        ...
        if mu != 0.0:
            axis.text(mu, -0.052, -7.0, f"{mu:g}", ha="center", va="top", fontsize=tick_fontsize)
    for xi in xi_ticks:
        axis.plot([-5.0, 5.0], [xi, xi], [0.0, 0.0], **axis_style)
        if xi != 0.0:
            axis.text(-11.0, xi, -5.0, f"{xi:g}", ha="right", va="center", fontsize=tick_fontsize)
    for temperature in temperature_ticks:
        axis.plot([-5.0, 5.0], [0.0, 0.0], [temperature, temperature], **axis_style)
        if temperature != 0.0:
            axis.text(-14.0, 0.0, temperature, f"{temperature:g}", ha="right", va="center", fontsize=tick_fontsize)

    axis.text(-10.0, -0.038, -7.0, "0", ha="right", va="top", fontsize=tick_fontsize)
    axis.text(416.0, 0.0, -7.0, r"$\mu_q$ [MeV]", ha="left", va="center", fontsize=12)
    axis.text(0.0, 0.59, -5.0, r"$\xi$ (dimensionless)", ha="left", va="top", fontsize=11)
    axis.text(0.0, 0.0, 241.0, "T [MeV]", ha="center", va="bottom", fontsize=12)
...
def _shade_quads_v5(
    ...
    *,
    ...
    """Use a stronger geometry-only directional light, not an observable map."""

    import numpy as np
    from matplotlib.colors import to_rgb

    base = np.asarray(to_rgb(base_color), dtype=float)
    light = np.asarray([0.58, -0.44, 0.70], dtype=float)
...
def render_figure(
    ...
    try:
        ...
        raise RuntimeError("matplotlib is required for the v5 phase render") from exc

    fig = plt.figure(figsize=(11.8, 9.0), dpi=600)
    axis = fig.add_subplot(111, projection="3d")
    axis.set_proj_type("ortho")
    _configure_axis(axis)
    _draw_inner_axes_v5(axis)
    ...
    axis.add_collection3d(
        Poly3DCollection(
            crossover_quads,
            facecolors=_shade_quads_v5(crossover_quads, "#d9792b"),
            **collection_kwargs,
        )
    )
    axis.add_collection3d(
        Poly3DCollection(
            maxwell_quads,
            facecolors=_shade_quads_v5(maxwell_quads, "#245b8f"),
            **collection_kwargs,
        )
    )
    ...
    if ordered_cep:
        endpoint = ordered_cep[-1]
        axis.text(
            number(endpoint, "mu_CEP_proxy_MeV") + 4.0,
            number(endpoint, "xi") + 0.015,
            number(endpoint, "T_midpoint_MeV") + 4.0,
            "CEP",
            color=CEP_LINE_COLOR,
            fontsize=10.5,
    ...
    axis.plot(mu_zero_mu, mu_zero_xi, mu_zero_T, color="white", linewidth=5.0, alpha=0.98)
    axis.plot(
        mu_zero_mu,
        mu_zero_xi,
        mu_zero_T,
    ...
    legend_handles = [
        Line2D([0], [0], color="#d9792b", linewidth=6, alpha=0.82, label="crossover"),
        Line2D([0], [0], color="#245b8f", linewidth=6, alpha=0.82, label="first-order (Maxwell)"),
    ...
    fig.suptitle("PNJL phase structure", fontsize=14, y=0.976)
    axis.set_xlim(-18.0, 420.0)
    axis.set_ylim(-0.60, 0.60)
    axis.set_zlim(-8.0, 242.0)
    axis.set_box_aspect((1.0, 1.0, 1.0))
    axis.view_init(elev=30.0, azim=-45.0)
    fig.subplots_adjust(left=0.0, right=1.0, bottom=0.0, top=0.85)
...
def build_plot_manifest(
    *,
    ...
    return {
        ...
        "asset_id": "pnjl_issue130_phase_reference_v5",
        "figure_family": "pnjl_phase_reference",
        "case_slug": "issue130_phase_reference_v5",
        "figure_version": "v5",
        "asset_kind": "figure",
        ...
        "generator": {
            **file_record(generator, root),
            ...
            "command": "render_issue130_phase_surface_v5.py",
            "runtime": {
...
def main() -> int:
    ...
    if output_root.exists():
        raise FileExistsError(f"refusing to overwrite v5 evidence root: {output_root}")
    ...
    crossover_quads, crossover_summary = build_surface_quads(
        crossover_groups, axis="mu_MeV", value_field="T_MeV", grid_points=args.grid_points, max_gap=26.0
    )
    maxwell_quads, maxwell_summary = build_surface_quads(
        maxwell_groups, axis="T_MeV", value_field="mu_MeV", grid_points=args.grid_points, max_gap=18.0
    )
    ...
    manifest = build_plot_manifest(
        root=root, output_png=output_png, output_svg=output_svg,
        generator=Path(__file__).resolve(), inputs=inputs,
    )
    ...
    _write_json(
        evidence_root / "decision.json",
        {
            ...
            "public_alias_replaced": False,
            "pdf_emitted": False,
            "supersedes": "data/outputs/figures/pnjl/phase_reference/issue130_phase_reference_v4/",
        },
    )
    (evidence_root / "README.md").write_text(
        "# Issue #130 phase-reference surface figure v5\n\n"
        "Display-only style candidate from the immutable derived tables. The three axes are "
        "mu_q [MeV], xi (dimensionless), and T [MeV] and meet at the inner physical origin. "
        "v5 uses an equal display aspect, orthographic projection, neutral directional shading, "
        "and very fine low-opacity surface edges for depth. These are visual effects only; no "
        "observable is encoded by color or edge tone. The CEP line is magenta and the crossover "
        "intersection with the mu_q=0 plane is a black dashed trace outside the legend. Only PNG "
        "and SVG are emitted; strict/derived data and the public alias are unchanged.\n",
        encoding="utf-8",
    )
    print(json.dumps({"output_root": str(output_root), "png": str(output_png), "svg": str(output_svg), "solver_called": False}, ensure_ascii=False, indent=2))
    return 0
