#!/usr/bin/env python3
"""Render the Issue #130 phase-reference v8 display layer.

The numerical strict/derived tables are immutable inputs.  This renderer only
builds a display mesh from local common support and records a boundary-
constrained crossover endpoint at the estimated CEP midpoint.  The endpoint
is a visualization-only row and is never written to the reference tables.
"""
...
def render_figure(
    ...
    try:
        ...
        raise RuntimeError("matplotlib is required for the v8 phase render") from exc
...
def build_plot_manifest(
    *,
    ...
    return {
        ...
        "asset_id": "pnjl_issue130_phase_surface_v8",
        ...
        "generator": {
            **file_record(generator, root),
            ...
            "command": "render_issue130_phase_surface_v8.py",
            "runtime": {
        ...
        "series": [
            ...
            },
            {
                ...
                "support_rule": "derived Maxwell rows only within adjacent xi common T support",
        ...
        "selection_rule": "render only finite derived rows; preserve source support and add explicit CEP display endpoint",
        "interpolation_policy": "local_common_support_mesh_plus_boundary_constrained_display_endpoint",
        ...
        "rendering": {
            ...
            "low_temperature_display": "common-support masked; no T=0 extrapolation",
        },
        "validation": {
            "finite": True,
            "duplicate_keys": True,
...
def main() -> int:
    ...
    if output_root.exists():
        raise FileExistsError(f"refusing to overwrite v8 evidence root: {output_root}")
    ...
    previous_public_sha = sha256(public_png) if public_png.is_file() else None
    previous_public_manifest = public_root / "plot_manifest.json"
    previous_public_manifest_sha = sha256(previous_public_manifest) if previous_public_manifest.is_file() else None
    ...
    package_manifest = build_plot_manifest(
        ...
        case_slug="issue130_phase_surface_render_v8",
    )
    ...
    public_manifest = build_plot_manifest(
        ...
        generator=Path(__file__).resolve(),
        inputs=source_inputs,
        case_slug="issue130_phase_reference_v1_public_v8_alias",
    )
    ...
    summary_rows = [
        ...
        {"surface": "first_order_coexistence_maxwell", **maxwell_summary, "display_policy": "masked_common_temperature_support"},
    ]
    write_csv(tables_root / "render_surface_summary.csv", list(summary_rows[0].keys()), summary_rows)

    generated_at = datetime.now(timezone.utc).isoformat()
    root_manifest = {
        "schema_version": "pnjl_issue130_phase_surface_render_v8",
        "generated_at_utc": generated_at,
        "calculation_sha": CALCULATION_SHA,
        "source_run_id": SOURCE_RUN_ID,
        "replay_run_id": SOURCE_REPLAY_ID,
        "source_layer": "data/reference/pnjl/issue130_phase_reference_v1/derived",
        "source_render_layer": "phase_surface_render_v1",
        "render_mode": "visualization_only_closed_candidate",
        "solver_called": False,
        "strict_and_derived_tables_unchanged": True,
        "previous_public_png_sha256": previous_public_sha,
        "previous_public_plot_manifest_sha256": previous_public_manifest_sha,
        ...
        "policies": {
            ...
            "maxwell_low_temperature": "masked common-support surface; no T=0 extrapolation",
            "triangulation": False,
    ...
    (output_root / "README.md").write_text(
        "# Issue #130 phase-surface render v8\n\n"
        "This is a display-only successor to the v1 phase-reference render. "
        ...
        "first-order (Maxwell) surface is built only from adjacent xi slices "
        "with common temperature support. No T=0 extrapolation or triangulation "
        "is used. The stable public PNG alias is intentionally written to the "
        "existing phase-reference figure path; the previous v1 source figure "
        "remains preserved under the v1 analysis package.\n",
        encoding="utf-8",
    )
    ...
    write_csv(
        ...
        [
            ...
            {
                "claim_id": "V8-RENDER-002",
                "status": "supported",
                "claim": "The plotted first-order surface is labelled as Maxwell coexistence and preserves low-temperature support masking.",
                "evidence": "tables/source_coverage_mask.csv; figures/plot_manifest.json",
                "boundary": "no T=0 extrapolation",
            },
            {
                "claim_id": "V8-RENDER-003",
                "status": "display_only",
    ...
    # stable public figure alias while preserving the v1 source-render record.
