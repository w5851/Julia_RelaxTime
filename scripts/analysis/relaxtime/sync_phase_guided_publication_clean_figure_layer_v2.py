#!/usr/bin/env python3
"""Mirror the semantic-correction publication-clean figures to the public layer.

The analysis builder owns ``docs/analysis/...publication_clean_v2``.  This
script performs a hash-checked, byte-preserving mirror to
``data/outputs/figures/.../publication_clean_v2`` and records the figure-layer
manifest.  It never touches raw results, registry files, or the v1 snapshot.
"""
...
FIGURE_LAYER_ROOT = (
    ...
    / "relaxtime"
    / "phase_guided_transport"
    / "phase_guided_transport_publication_clean_figure_layer_v2"
)
...
def load_manifest() -> dict[str, Any]:
    ...
    if payload.get("marker_contract") != "phase_endpoint_gap_v2":
        raise ValueError("analysis plot manifest is not the v2 endpoint-gap contract")
    return payload


def load_package_manifest() -> dict[str, Any]:
    if not ANALYSIS_PACKAGE_MANIFEST.exists():
        raise FileNotFoundError(f"missing analysis package manifest: {ANALYSIS_PACKAGE_MANIFEST}")
...
def main() -> None:
    ...
    public_plot_manifest = dict(analysis_manifest)
    public_plot_manifest["figure_layer_role"] = "publication_clean_display_mirror"
    public_plot_manifest["source_analysis_plot_manifest"] = relpath(ANALYSIS_PLOT_MANIFEST)
    public_plot_manifest["source_analysis_plot_manifest_sha256"] = sha256_file(ANALYSIS_PLOT_MANIFEST)
    public_plot_manifest["figures"] = assets
    public_plot_manifest["rendering_semantics"] = (
        str(public_plot_manifest.get("rendering_semantics", ""))
    ...
    layer_manifest = {
        "schema": "phase_guided_transport_publication_clean_figure_layer_manifest_v2",
