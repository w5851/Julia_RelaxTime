...
const OUT_CSV = raw"D:\Desktop\Temp\relaxtime_t200_window\t200_detm_im_upstream_trace.csv"
...
function main()
    ...
    try
        println(
            io,
            "scenario,process,xi,ds,s_value,k0,k_norm,pi_uu_im,pi_ss_im,im_M00M88,im_M08sq,detM_im",
        )
        ...
        for xi in XI_LIST
            ...
            for ds in DS_POINTS
                ...
                Π_uu_re, Π_uu_im = Main.PolarizationCache.polarization_aniso_cached(
                    :P, cms.k0, cms.k,
                    st.quark_params.m.u, st.quark_params.m.u,
                    st.quark_params.μ.u, st.quark_params.μ.u,
                    st.thermo_params.T, st.thermo_params.Φ, st.thermo_params.Φbar, st.thermo_params.ξ,
                    st.quark_params.A.u, st.quark_params.A.u,
                    0,
                )
                Π_ss_re, Π_ss_im = Main.PolarizationCache.polarization_aniso_cached(
                    :P, cms.k0, cms.k,
                    st.quark_params.m.s, st.quark_params.m.s,
                    st.quark_params.μ.s, st.quark_params.μ.s,
                    st.thermo_params.T, st.thermo_params.Φ, st.thermo_params.Φbar, st.thermo_params.ξ,
                    st.quark_params.A.s, st.quark_params.A.s,
                    2,
                )
    ...
    println("Wrote upstream trace to: " * OUT_CSV)
end
