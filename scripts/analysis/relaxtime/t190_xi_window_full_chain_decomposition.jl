...
function main()
    T_MeV = 190.0
    muB_MeV = 0.0
    xi_values = collect(range(-0.2, 0.0; length=11))
    ...
    out_detail = raw"D:\Desktop\Temp\relaxtime_t190_window\t190_xi_window_full_chain_detail.csv"
    out_summary = raw"D:\Desktop\Temp\relaxtime_t190_window\t190_xi_window_full_chain_summary.csv"
    out_adjacent = raw"D:\Desktop\Temp\relaxtime_t190_window\t190_xi_window_adjacent_transition_summary.csv"
    ...
    try
        ...
        println(io_adj, "process,xi_left,xi_right,ratio_area_sigma,right_over_left_ratio_sigma_over_T,ratio_area_abs_Dmixed_sq,ratio_area_abs_detM_sq,ratio_area_abs_DsP_mixed_sq,ratio_area_abs_DsP_simple_sq,ratio_area_M_s_sq,ratio_area_M_t_sq,ratio_area_M_interf,ratio_area_abs_JMJ_sq,ratio_area_detK_plus")
        ...
        t190_rows = filter(r -> Float64(r.T_MeV) == 190.0 && Float64(r.muB_MeV) == 0.0 && Float64(r.xi) >= -0.2 && Float64(r.xi) <= 0.0, rows)
        sort!(t190_rows, by=r -> Float64(r.xi))
        sigma_over_T_by_xi = Dict{Float64, Float64}()
        for r in t190_rows
            sigma_over_T_by_xi[Float64(r.xi)] = Float64(r.sigma_over_T)
        end
