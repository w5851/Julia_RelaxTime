...
function main()
    ...
    processes = Symbol[:udbar_to_udbar, :uubar_to_ddbar, :uubar_to_uubar]
    ds = 1.0e-8

    out = raw"D:\Desktop\Temp\relaxtime_t200_window\t200_target_channels_denominator_trace.csv"
    ensure_parent_dir(out)

    io = open(out, "w")
    try
        println(io, "process,xi,channel,s_minus_sth,k0,k_norm,meson_simple_ref,re_den_simple,im_den_simple,has_mixed,re_detM,im_detM,abs_D_simple_sq,abs_D_mixed_sq,abs_D_total_sq,M2_total,M_s_sq,M_t_sq,M_interf")
        for process in processes
            for xi in xi_values
                ...
                for channel in (:s, :t)
                    ...
                    simple = simple_p_branch_for_channel(process, channel, cms.k0, cms.k, st.quark_params, st.thermo_params, st.K_coeffs)
                    mixed = mixed_p_branch_for_channel(process, channel, cms.k0, cms.k, st.quark_params, st.thermo_params, st.K_coeffs)
