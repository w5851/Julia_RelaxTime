...
#### 6.6 性能测试（基准：50000次迭代的平均耗时，A值已预计算）

> **测试方法说明**：
> - 使用 50000 次迭代 + 10000 次预热，确保 JIT 编译完成和 CPU 缓存预热
> - 每次测试前执行 GC.gc() 清理垃圾回收
> - A 值预先计算并作为参数传入，排除重复计算开销
> - 使用 time_ns() 进行纳秒级计时，测量精度约 1μs
> - 测试数据来自 `test_performance_final.jl` 和 `test_performance_verification.jl`

**各向同性情况 (ξ = 0)：**
- Π_P 通道：平均 **0.028 ms/次**
- Π_S 通道：平均 **0.027 ms/次**

**各向异性情况 (ξ = 0.5)：**
- Π_P 通道：平均 **0.079 ms/次**
- Π_S 通道：平均 **0.080 ms/次**

**性能分析：**
- ✓ 各向异性计算约为各向同性的 **2.8-3.0 倍**（合理，因为有额外的 B0_correction 积分）
- ✓ 所有情况下单次计算耗时 < 0.09 ms，性能优秀
- ✓ P 通道和 S 通道性能完全相同（差异 <4%，在测量误差范围内）
- ✓ 即使在各向异性情况下，千次调用也只需约 0.08 秒
- ✓ **A 值预计算优化**：将 A 函数作为传入参数而非函数内部重复计算，避免了不必要的高斯积分开销
- ✓ **num_s_quark=1 的影响**：使用对称平均（2×B0）导致各向同性情况下耗时约为单次 B0 的 2.7 倍
- ✓ **基准对比**：单次 B0 ≈ 0.010 ms，2×B0 ≈ 0.026 ms，polarization(ξ=0) ≈ 0.028 ms，完全符合理论预期

**计算成本对比：**

| 通道 | ξ = 0 (ms) | ξ = 0.5 (ms) | 增加倍数 | 额外耗时 (ms) |
|------|-----------|-------------|---------|--------------|
| Π_P  | 0.028     | 0.079       | 2.79×   | +0.050       |
| Π_S  | 0.027     | 0.080       | 2.95×   | +0.053       |
...
- ✅ 当前性能已非常优秀（单次 < 0.09 ms，扫描100个点仅需 ~8 ms）
...
**性能表现：**
- 各向同性：~0.028 ms/次（足够快，可用于实时计算）
- 各向异性：~0.080 ms/次（仍然很快，ξ 修正带来的额外开销约 ~0.052 ms）
- 性能开销主要来自 B0_correction 的额外积分，但仍在完全可接受范围内

**实用性评估：**
- ✅ 单点计算：实时响应（< 0.1 ms）
- ✅ 参数扫描：毫秒级完成（100点 ~8 ms）
- ✅ 大规模计算：秒级（10⁶ 点 ~80 秒）
- ✅ 适合嵌入到更大的计算流程中
...
5. **性能优异**：
   - 各向同性：~0.028 ms/次
   - 各向异性：~0.080 ms/次（增加约 2.9 倍，性能开销可接受）
   - A 值作为传入参数的设计显著提升了效率
   - 适用于大规模参数扫描和实时计算场景
...
- **性能测试基准**：50000 次迭代 + 10000 次预热，测量精度 ~1μs

### 性能基准

基于 50000 次迭代测试的平均单次调用耗时（**A 值已预计算并作为参数传入**）：

| 函数调用 | 平均耗时 (ms) | 标准偏差估计 | 吞吐量 (次/秒) |
|---------|--------------|-------------|---------------|
| `polarization_aniso(:P, ξ=0)` | 0.028 | ~0.001 | ~35,400 |
| `polarization_aniso(:S, ξ=0)` | 0.027 | ~0.001 | ~36,800 |
| `polarization_aniso(:P, ξ=0.5)` | 0.079 | ~0.002 | ~12,700 |
| `polarization_aniso(:S, ξ=0.5)` | 0.080 | ~0.002 | ~12,500 |

**实际应用场景估算：**
- 单点计算：< 0.09 ms ✓
- ξ 参数扫描（10点）：~0.8 ms ✓
- (k, ξ) 二维扫描（10×10）：~8 ms ✓
- (k₀, k, ξ) 三维扫描（10×10×10）：~79 ms ✓
...
- ✅ **关键性能指标**：单次 B0 调用约 0.010 ms，2×B0（对称平均）约 0.026 ms，完整 polarization_aniso(ξ=0) 约 0.028 ms
...
- ✅ 各向同性：~0.028 ms/次（性能优秀，比之前测试更准确）
- ✅ 各向异性：~0.080 ms/次（主要瓶颈在 B0_correction 积分）
- ✅ 为实际代码使用提供了清晰的优化示例
- ✅ **重要发现**：之前测试显示的 0.05 ms 实际上是 num_s_quark=1 导致的 2×B0 调用（对称平均），完全符合理论预期
...
**最后更新**：2025年11月11日  
**变更日志**：
- v1.2 (2025-11-11): 更新性能测试数据，使用 50000 次迭代 + 10000 次预热，修正了之前的测量误差
- v1.1 (2025-11-10): 添加 A 值预计算优化
